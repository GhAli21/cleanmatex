...
# Confirm action
if (-not $SkipConfirm) {
    Write-Host "âš ï¸  WARNING: This will DESTROY all data in the local database!" -ForegroundColor Red
    Write-Host ""
...
Write-Host "  âœ“ Production schema loaded" -ForegroundColor White
Write-Host "  âœ“ Lookup tables seeded" -ForegroundColor White

if (-not $Tenant2Only) {
    Write-Host "  âœ“ Demo Tenant #1 loaded" -ForegroundColor White
}
if (-not $Tenant1Only) {
    Write-Host "  âœ“ Demo Tenant #2 loaded" -ForegroundColor White
}

if ($LASTEXITCODE -eq 0) {
    Write-Host "  âœ“ Admin users created" -ForegroundColor White
}

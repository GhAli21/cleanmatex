---
name: code-reviewer
description: Use only when explicitly requested for scoped code-reviewer work. Avoid broad scans and nested agents.
version: 1.1.0
model: sonnet
---
...
Use this agent only when explicitly requested for scoped **code-reviewer** work.
...
- For web-admin UI diffs: flag missing `cmxMessage` / `useMessage()` when applicable, and flag new legacy toast/`alert()` usage (`docs/dev/rules/cmx-message.md`).
...
```text
.claude/agent-references/code-reviewer-reference-original.md
```

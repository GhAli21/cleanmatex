...
**Migration handling:** Generate the `sys_components_cd` migration SQL file but DO NOT apply it. Stop and ask the user to review before they apply it. If the menu entry introduces new permissions, ensure each new permission code has its own DB migration (TS-only permissions are incomplete).

...
effort: low
references:
  - ../../../.claude/skills/create-feature-flag/reference.md
  - ../../../.claude/skills/create-feature-flag/CHANGELOG.md
  - ../../../CLAUDE.md
agents:
---
...
`plan_binding_type = 'plan_bound'`), then syncs the web-admin TypeScript catalog.

**This skill shares its reference content with the Claude Code version** at
`.claude/skills/create-feature-flag/` — `reference.md` (full templates/examples/troubleshooting) and
`CHANGELOG.md` are the same files, not copies, so both harnesses stay in sync automatically. Only this
`SKILL.md` entry point is Codex-specific (tool/MCP names below). Do not fork `reference.md` — edit the
one under `.claude/skills/create-feature-flag/` and both tools pick up the change.

**Provenance**: originally copied verbatim from `cleanmatexsaas` (which owns the HQ feature-flag admin
UI/API), then corrected against the live remote DB and the real
...
- Treat the **remote** Supabase DB as authoritative for schema/uniqueness checks — use the
  `supabase_remote` MCP server configured in `.codex/config.toml` (not `supabase_local`, which is
  minimal/unrepresentative dev data).
...
- **NEVER apply the migration** — create the `.sql` file, then STOP and ask the user to review and apply.
  This project (`cleanmatex`) owns migrations; do not use any MCP or CLI to execute them.
...
2. Validate against the remote DB (supabase_remote MCP): flag_key uniqueness, plan_code existence.
...
Full workflow, SQL templates (upsert `ON CONFLICT ... DO UPDATE`), worked examples, placeholder table,
and troubleshooting live in the shared file — read it before generating any migration SQL:

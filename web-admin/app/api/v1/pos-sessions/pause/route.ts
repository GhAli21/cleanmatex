...
import { pausePosSession } from '@/lib/services/pos-session.service';
...
export async function POST(request: NextRequest) {
  ...
  const auth = await requirePermission('pos_session:pause_resume')(request);
  ...
  try {
    const result = await pausePosSession({

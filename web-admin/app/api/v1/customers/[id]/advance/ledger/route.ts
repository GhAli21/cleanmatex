...
export async function GET(
  ...
  try {
    ...
    if (!advance) return NextResponse.json({ success: true, data: { entries: [], total: 0 } });

    const [entries, total] = await Promise.all([
      withTenantContext(tenantId, () =>
        prisma.org_advance_txn_dtl.findMany({
          where:   { tenant_org_id: tenantId, advance_id: advance.id },
      ...
      withTenantContext(tenantId, () =>
        prisma.org_advance_txn_dtl.count({ where: { tenant_org_id: tenantId, advance_id: advance.id } })
      ),
    ]);

    return NextResponse.json({ success: true, data: { entries, total, page, limit } });
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Failed to fetch advance ledger';

...
export default async function ErpLiteApPage({
  ...
  const isEnabled = await currentTenantCan(FEATURE_FLAG_KEYS.ERP_LITE_AP_ENABLED);

  if (!isEnabled) {
    return (
      <ErpLitePageGuard feature={FEATURE_FLAG_KEYS.ERP_LITE_AP_ENABLED} permissions={['erp_lite_ap:view']}>
        {null}
      </ErpLitePageGuard>
    );
  }
  ...
  let snapshot: ErpLiteApDashboardSnapshot = {
    supplier_list: [],
    ap_invoice_list: [],
    ap_payment_list: [],
    ap_aging_list: [],
  ...
  try {
    snapshot = await ErpLiteV2Service.getApDashboardSnapshot(locale);
  } catch (loadFailure) {
    loadError =

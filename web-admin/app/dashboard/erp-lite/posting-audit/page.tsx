...
export default async function ErpLitePostingAuditPage({ searchParams }: PostingAuditPageProps) {
  ...
  const isEnabled = await currentTenantCan(FEATURE_FLAG_KEYS.ERP_LITE_POST_AUDIT_ENABLED)
  if (!isEnabled) {
    return (
      <ErpLitePageGuard feature={FEATURE_FLAG_KEYS.ERP_LITE_POST_AUDIT_ENABLED} permissions={['erp_lite_post_audit:view']}>
        {null}
      </ErpLitePageGuard>
    )
  }

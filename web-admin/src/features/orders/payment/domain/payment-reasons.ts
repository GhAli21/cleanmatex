...
export const PAYMENT_REASON = {
  ...
  // CLF cash-drawer ledger gate codes (lib/constants/cash-drawer.ts
  // CASH_LEDGER_ERRORS) — thrown inside the posting transaction since W2
  // removed the planner's pre-transaction session check.
...
export const SERVER_ERROR_TO_REASON: Record<string, PaymentReasonCode> = {
  ...
  // CLF cash-drawer ledger gate codes — surfaced on the drawer guard.
  CASH_DRAWER_REQUIRED: PAYMENT_REASON.CASH_DRAWER_REQUIRED,
  CASH_DRAWER_SESSION_NOT_OPEN: PAYMENT_REASON.CASH_DRAWER_SESSION_NOT_OPEN,
  DRAWER_SESSION_CLOSING: PAYMENT_REASON.DRAWER_SESSION_CLOSING,

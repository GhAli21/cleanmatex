...
export default function AdjustStockModal({ item, onClose, onSuccess, branchId: initialBranchId, branches = [] }: AdjustStockModalProps) {
  ...
  return (
    <CmxDialog open onOpenChange={(o) => { if (!o) onClose(); }}>
      <CmxDialogContent className="w-full max-w-md mx-4">
        <CmxDialogHeader>
          <CmxDialogTitle>{t('actions.adjustStock')}</CmxDialogTitle>
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-6 py-4 space-y-4">
            ...
            {/* Branch dropdown — always selectable */}
            {hasBranches ? (
              <div>
          ...
          <CmxDialogFooter>
            <CmxButton type="button" variant="secondary" onClick={onClose} disabled={saving}>
              {tc('cancel')}
            </CmxButton>
            <CmxButton type="submit" disabled={saving || !selectedBranchId}>

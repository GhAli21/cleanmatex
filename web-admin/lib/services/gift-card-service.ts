...
/**
 * @deprecated Use refundGiftCardTx — retained for backward compatibility with
 * order-cancel-service which calls this using card number.
 */
export async function refundToGiftCardTx(
  ...
  return refundGiftCardTx(tx, {
    ...
    processedBy: params.processedBy,
    tenantOrgId: params.tenantOrgId,
    idempotencyKey: `${params.orderId}:refund`,
  });
}

/**
 * Standalone gift card refund (opens its own transaction).
 * @deprecated Prefer calling refundGiftCardTx inside a shared transaction.
 */
export async function refundToGiftCard(
  ...
  const tenantId = await getTenantIdFromSession();
  if (!tenantId) {
    return { success: false, newBalance: 0, actualRefundAmount: 0, error: 'Unauthorized: Tenant ID required' };
  }

  return withTenantContext(tenantId, async () => {
    try {
      const result = await prisma.$transaction((tx) =>
        refundToGiftCardTx(tx, { cardNumber, amount, orderId, invoiceId, reason, processedBy, tenantOrgId: tenantId })
      );

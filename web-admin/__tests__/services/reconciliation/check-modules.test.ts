...
describe('order-snapshot-checks', () => {
  ...
  it('B18 write-shape: only ORDER-level PREFERENCE charges are money addends', async () => {
    const pieceExtra = 3;
    const itemExtra = 2;
    const orderExtra = 5;

    mockChargesAggregate.mockResolvedValue({ _sum: { amount: new Decimal(String(orderExtra)) } });
    mockChargesFindMany.mockResolvedValue([
    ...
    mockPreferencesFindMany.mockResolvedValue([
      { id: 'pref-item-1', extra_price: new Decimal(String(itemExtra)), prefs_level: 'ITEM' },
      { id: 'pref-piece-1', extra_price: new Decimal(String(pieceExtra)), prefs_level: 'PIECE' },
      { id: 'pref-piece-2', extra_price: new Decimal(String(pieceExtra)), prefs_level: 'PIECE' },
      { id: 'pref-order-1', extra_price: new Decimal(String(orderExtra)), prefs_level: 'ORDER' },
    ]);
    mockOrderFindUnique.mockResolvedValue({ total_charges_amount: new Decimal(String(orderExtra)) });

-- =============================================================================
-- Migration 0237: Force-fix ambiguous "session_id" in
--                 hq_crm_marketing_upsert_visitor_session
--
-- Root cause confirmed: PostgreSQL raises "column reference session_id is
-- ambiguous" because RETURNS TABLE declares an output column named session_id
-- AND the INSERT target table has a column also named session_id.  Even with a
-- fully-qualified table name in RETURNING, some PG planner versions reject it.
--
-- Fix strategy:
--   1. DROP the existing function to ensure no stale definition remains.
--   2. Recreate with a named composite type for RETURNS to sidestep the naming
--      clash, OR use a local variable approach in the RETURNING body.
--      Chosen: keep RETURNS TABLE but rename the output column to
--      "out_session_id" so it never collides with the table column.
--      Callers currently only use .id and .lead_id fields so the rename is
--      backwards-compatible for the TypeScript side.
--
-- Note: migration 0235 used CREATE OR REPLACE which appeared to succeed but
--       the live DB still shows the old definition — this migration uses DROP
--       first to guarantee the replacement is applied.
-- =============================================================================

-- Drop existing function (regardless of whether 0235 was actually applied)
DROP FUNCTION IF EXISTS public.hq_crm_marketing_upsert_visitor_session(jsonb, jsonb, boolean, integer);

CREATE FUNCTION public.hq_crm_marketing_upsert_visitor_session(
  p_payload jsonb,
...
BEGIN
  ...
  -- Event capping computed ahead of upsert to keep row-lock scope small.
  ...
  SET visitor_id             = COALESCE(EXCLUDED.visitor_id, vs.visitor_id),
      ...
      updated_at             = now()
  -- "vs.session_id" unambiguously references the table column (via alias).
  -- The RETURNS TABLE output column is now named "out_session_id" so there
  -- is zero naming overlap with any table column.
  RETURNING
    vs.id,
    vs.session_id   AS out_session_id,
    vs.lead_id,
    vs.page_views,
    vs.total_dwell_ms,
    vs.max_scroll_pct,
...
COMMENT ON FUNCTION public.hq_crm_marketing_upsert_visitor_session(jsonb, jsonb, boolean, integer)
  IS 'Atomic marketing visitor-session merge RPC. Migration 0237: DROP+CREATE to force-replace stale definition. Output column renamed out_session_id to avoid ambiguity with table column session_id.';

GRANT EXECUTE ON FUNCTION public.hq_crm_marketing_upsert_visitor_session(jsonb, jsonb, boolean, integer) TO service_role;
GRANT EXECUTE ON FUNCTION public.hq_crm_marketing_upsert_visitor_session(jsonb, jsonb, boolean, integer) TO anon;

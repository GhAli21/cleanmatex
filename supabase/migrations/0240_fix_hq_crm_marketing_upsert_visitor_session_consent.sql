-- =============================================================================
-- Migration 0240: Fix consent_level coalesce bug in visitor session upsert
--
-- Root cause confirmed: In migration 0237, the ON CONFLICT DO UPDATE clause
-- for consent_level used COALESCE(..., 0). When an anonymous user navigates
-- (generating an exit or second view event without consent), the payload omits
-- consent_level (NULL). The COALESCE turned that NULL into 0, which the UI
-- then incorrectly displayed as "Consented L0".
--
-- Fix strategy:
--   Change the UPDATE line to use:
--     consent_level = GREATEST(vs.consent_level, EXCLUDED.consent_level)
--   This correctly preserves NULL if both are NULL, and takes the highest 
--   level if consent was granted.
-- =============================================================================
...
COMMENT ON FUNCTION public.hq_crm_marketing_upsert_visitor_session(jsonb, jsonb, boolean, integer)
  IS 'Atomic marketing visitor-session merge RPC. Migration 0240: Fixed consent_level COALESCE bug that incorrectly cast NULL to 0 on update.';
...
-- Repair existing corrupted rows where consent_level was accidentally set to 0
UPDATE public.hq_crm_marketing_visitor_sessions
SET consent_level = NULL
WHERE consent_level = 0;

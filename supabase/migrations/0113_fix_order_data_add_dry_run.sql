-- ==================================================================
-- Migration: 0113_fix_order_data_add_dry_run
-- Purpose: Add p_dry_run to fix_order_data — check only, no writes; show what fixes needed
-- ==================================================================

BEGIN;

DROP FUNCTION IF EXISTS fix_order_data(UUID, TEXT[], UUID);
  ...
  IF v_run_complete_pieces THEN
    ...
    BEGIN
      ...
      ELSIF p_dry_run THEN
        v_step_summary := v_items_adjusted || ' items would be adjusted. ' ||
          v_pieces_added || ' pieces would be added, ' || v_pieces_removed || ' would be removed.';
      ELSE
...
COMMENT ON FUNCTION fix_order_data(UUID, TEXT[], UUID, BOOLEAN) IS
  'Fix order data: when p_dry_run true only check and return what would be fixed; else apply. complete_order_item_pieces: sync pieces to quantity. Returns overall, steps[] (step_id, status, summary, details with item_results), dry_run.';

GRANT EXECUTE ON FUNCTION fix_order_data(UUID, TEXT[], UUID, BOOLEAN) TO service_role;
GRANT EXECUTE ON FUNCTION fix_order_data(UUID, TEXT[], UUID, BOOLEAN) TO authenticated;

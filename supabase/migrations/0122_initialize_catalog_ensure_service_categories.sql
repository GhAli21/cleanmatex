-- ==================================================================
-- 0122_initialize_catalog_ensure_service_categories.sql
-- Purpose: Fix fk_org_prod_ctg violation when initializing tenant catalog.
--   Root cause: org_product_data_mst requires (tenant_org_id, service_category_code)
--   to exist in org_service_category_cf. If the tenant has no service categories
--   (e.g. created outside full init flow), the insert fails.
--   Fix: Ensure required service categories exist in org_service_category_cf
...
BEGIN
  ...
  -- STEP 1: Ensure all service categories used by templates exist in org_service_category_cf
  -- This fixes fk_org_prod_ctg violation when tenant has no/missing categories
  INSERT INTO org_service_category_cf (tenant_org_id, service_category_code, rec_order)
  SELECT p_tenant_org_id, s.service_category_code, COALESCE(s.rec_order, 1)
  FROM sys_service_category_cd s
  WHERE s.is_active = true
    AND EXISTS (
      SELECT 1 FROM sys_service_prod_templates_cd t
      ...
      WHERE t.service_category_code = s.service_category_code
        AND t.is_to_seed = true
  ...
  -- STEP 2: Insert only products that don't already exist for this tenant
...
COMMENT ON FUNCTION initialize_tenant_product_catalog IS 'Initialize tenant product catalog from templates; ensures service categories exist (fixes fk_org_prod_ctg), skips existing products (idempotent)';

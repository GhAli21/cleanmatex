-- ==================================================================
-- 0118_fix_initialize_tenant_product_catalog_seed_filter.sql
-- Purpose: Fix "operator does not exist: text ? character varying" error
-- Root cause: seed_options->>'required_for' returns TEXT, but ? operator
--   requires JSONB on the left. Use @> (contains) on the JSONB array instead.
-- ==================================================================
...
COMMENT ON FUNCTION initialize_tenant_product_catalog IS 'Initialize tenant product catalog from templates filtered by business type (fixed seed_options check)';

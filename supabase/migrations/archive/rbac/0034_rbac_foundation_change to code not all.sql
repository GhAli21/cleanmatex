...
CREATE TABLE IF NOT EXISTS sys_auth_permissions (
  ...
  code TEXT NOT NULL PRIMARY KEY, -- UNIQUE,              -- 'orders:read', 'orders:create', 'workflow:transition', etc.
...
CREATE TABLE IF NOT EXISTS sys_auth_roles (
  ...
  code TEXT NOT NULL PRIMARY KEY, --UNIQUE,              -- 'super_admin', 'tenant_admin', 'operator', 'viewer', etc.
  ...
  --CONSTRAINT check_system_role_code CHECK (NOT is_system OR code IN ('super_admin', 'admin', 'tenant_admin', 'branch_manager', 'operator', 'viewer'))
);
...
CREATE TABLE IF NOT EXISTS org_auth_user_roles (
  ...
  user_id UUID NOT NULL REFERENCES  org_users_mst(id) ON DELETE CASCADE,-- auth.users(id) ON DELETE CASCADE,
  ...
  --UNIQUE (user_id, tenant_org_id, role_id)
   PRIMARY KEY(user_id, tenant_org_id, role_id)
);
...
CREATE TABLE IF NOT EXISTS org_auth_user_resource_roles (
  ...
  role_code UUID NOT NULL REFERENCES sys_auth_roles(code) ON DELETE CASCADE,
  is_active BOOLEAN NOT NULL DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  created_by VARCHAR(120),
  updated_at TIMESTAMPTZ,
  ...
  -- One user can have one role per resource
   PRIMARY KEY (user_id, tenant_org_id, resource_type, resource_id, role_code)
);
...
CREATE TABLE IF NOT EXISTS org_auth_user_permissions (
  ...
  user_id UUID NOT NULL REFERENCES  org_users_mst(id) ON DELETE CASCADE,-- auth.users(id) ON DELETE CASCADE,
  tenant_org_id UUID NOT NULL REFERENCES org_tenants_mst(id) ON DELETE CASCADE,
  permission_code UUID NOT NULL REFERENCES sys_auth_permissions(code) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS org_auth_user_resource_permissions (
  ...
  -- One override per user per permission per resource
   PRIMARY KEY (user_id, tenant_org_id, resource_type, resource_id, permission_id)
);

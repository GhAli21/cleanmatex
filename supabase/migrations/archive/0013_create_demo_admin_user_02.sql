...
-- Purpose: Create demo2 admin user in Supabase Auth automatically
...
-- ==================================================================
-- CREATE DEMO2 ADMIN USER
-- ==================================================================

DO $$
DECLARE
  v_demo_tenant_id UUID := '20000002-2222-2222-2222-222222222221';
  v_demo_email TEXT := 'admin@Demo2-laundry2.local';
  v_demo_password TEXT := 'Admin123';
  v_demo_display_name TEXT := 'Org2 Admin';
  v_result UUID;
BEGIN
  -- Check if Demo2 tenant exists
  IF NOT EXISTS (SELECT 1 FROM org_tenants_mst WHERE id = v_demo_tenant_id) THEN
    RAISE EXCEPTION 'Demo2 tenant not found. Run seed migration first (0013_seed_auth_demo_02.sql)';
  END IF;

  -- Create and link Demo2 admin user
  BEGIN
    ...
    RAISE NOTICE E'\n========================================';
    RAISE NOTICE '✅ Demo2 Admin User Created Successfully';
    RAISE NOTICE '========================================';
...
BEGIN
  ...
  FROM auth.users
  WHERE email = 'admin@Demo2-laundry.local';
  ...
  IF v_user_count > 0 AND v_link_count > 0 THEN
    RAISE NOTICE '✅ Validation passed: Demo2 admin user created and linked';
...
-- DELETE FROM org_users_mst WHERE tenant_org_id = '20000002-2222-2222-2222-222222222221';
-- DELETE FROM auth.users WHERE email = 'admin@Demo2-laundry.local';

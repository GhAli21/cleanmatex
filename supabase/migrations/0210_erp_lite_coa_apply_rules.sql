-- ==================================================================
-- Migration: 0210_erp_lite_coa_apply_rules.sql
-- Purpose: Upgrade ERP-Lite template validation and tenant materialization
--          for hardened COA lineage, manual reapply modes, and safer usage
--          mapping resolution
-- Project: cleanmatex (source of truth for shared DB migrations)
-- Notes:
--   - Keeps explicit HQ-driven apply only
--   - Adds DB-level validation for manual apply/reapply workflows
--   - Does NOT recreate automatic tenant-insert provisioning
-- ==================================================================

BEGIN;

ALTER TABLE public.org_fin_tpl_apply_log
  DROP CONSTRAINT IF EXISTS chk_oftal_mode;

ALTER TABLE public.org_fin_tpl_apply_log
  ADD CONSTRAINT chk_oftal_mode CHECK (
    apply_mode_code IN (
      'AUTO_INIT',
      'ERP_ENABLE',
      'MANUAL',
...
CREATE OR REPLACE FUNCTION public.validate_fin_tpl_for_tnt(
  p_tenant_id UUID,
...
BEGIN
  IF NOT EXISTS (
    ...
    RETURN QUERY
    SELECT 'BLOCKER'::VARCHAR, 'TENANT_NOT_FOUND'::VARCHAR, 'Tenant not found or inactive'::TEXT;
    RETURN;
  END IF;

  IF p_tpl_pkg_id IS NULL THEN
    SELECT r.tpl_pkg_id, r.tpl_pkg_code
    INTO v_tpl_pkg_id, v_tpl_pkg_code
    FROM public.resolve_fin_tpl_for_tnt(p_tenant_id) r;
  ELSE
    SELECT p.tpl_pkg_id, p.tpl_pkg_code
    INTO v_tpl_pkg_id, v_tpl_pkg_code
  ...
  IF v_tpl_pkg_id IS NULL THEN
    RETURN QUERY
    SELECT 'BLOCKER'::VARCHAR, 'TPL_NOT_RESOLVED'::VARCHAR, 'No published ERP-Lite template resolved for the tenant'::TEXT;
    RETURN;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RETURN QUERY
    SELECT 'BLOCKER'::VARCHAR, 'COA_TEMPLATE_MISSING'::VARCHAR, 'Resolved package has no active COA template header'::TEXT;
  END IF;

  RETURN QUERY
  SELECT
    'BLOCKER'::VARCHAR,
    'USAGE_BINDING_MISSING'::VARCHAR,
    'One or more template usage mappings are not bound to a template COA line'::TEXT
  WHERE EXISTS (
    SELECT 1
  ...
  SELECT
    'BLOCKER'::VARCHAR,
    'REQ_USAGE_NOT_POSTABLE'::VARCHAR,
    'A required usage mapping points to a non-postable or inactive template line'::TEXT
  WHERE EXISTS (
    SELECT 1
    FROM public.sys_fin_usage_tpl_dtl u
    JOIN public.sys_fin_coa_tpl_dtl d
      ON d.coa_tpl_line_id = u.coa_tpl_line_id
    WHERE u.tpl_pkg_id = v_tpl_pkg_id
      AND u.is_active = true
      AND u.rec_status = 1
      AND u.is_required = true
      AND (
        d.is_postable = false
  ...
  SELECT
    'BLOCKER'::VARCHAR,
    'TENANT_CODE_CONFLICT'::VARCHAR,
    'Tenant already has one or more account codes that conflict with the incoming template lineage'::TEXT
  WHERE EXISTS (
    SELECT 1
    FROM public.sys_fin_coa_tpl_mst h
    JOIN public.sys_fin_coa_tpl_dtl d
    ...
    WHERE h.tpl_pkg_id = v_tpl_pkg_id
      ...
      AND d.rec_status = 1
      AND (
        a.source_tpl_line_id IS DISTINCT FROM d.coa_tpl_line_id
        OR a.source_tpl_pkg_id IS DISTINCT FROM v_tpl_pkg_id
      )
  );
  ...
  SELECT
    'BLOCKER'::VARCHAR,
    'TENANT_SEEDED_DIVERGENCE'::VARCHAR,
    'Tenant has seeded template accounts whose structural metadata diverged from the resolved template'::TEXT
  WHERE EXISTS (
    SELECT 1
    FROM public.org_fin_acct_mst a
    JOIN public.sys_fin_coa_tpl_dtl d
      ON d.coa_tpl_line_id = a.source_tpl_line_id
    WHERE a.tenant_org_id = p_tenant_id
      AND a.source_tpl_pkg_id = v_tpl_pkg_id
      AND a.is_active = true
      AND a.rec_status = 1
      AND (
        a.acc_type_id IS DISTINCT FROM d.acc_type_id
        OR a.parent_account_id IS DISTINCT FROM (
          SELECT p.id
          FROM public.org_fin_acct_mst p
          WHERE p.tenant_org_id = a.tenant_org_id
            AND p.source_tpl_line_id = d.parent_tpl_line_id
          LIMIT 1
        )
      )
  );
  ...
  SELECT
    'INFO'::VARCHAR,
    'TPL_VALIDATED'::VARCHAR,
    'Template validation passed with no blocking database issues'::TEXT;
END;
$$;
...
BEGIN
  ...
  SELECT
    ...
    'system_seed',
    'Migration 0210 - ERP-Lite template apply root lines',
    1,
    d.rec_order,
    true
  FROM public.sys_fin_coa_tpl_mst h
  ...
  LOOP
    ...
    SELECT
      ...
      d.allow_tenant_children,
      'system_seed',
      'Migration 0210 - ERP-Lite template apply child lines',
      1,
      d.rec_order,
      true
    FROM public.sys_fin_coa_tpl_mst h
  ...
  IF COALESCE(p_apply_mode, 'MANUAL') IN ('REAPPLY_SYNC', 'REAPPLY_FULL') THEN
    ...
    SET
      ...
      updated_info = 'Migration 0210 - ERP-Lite template metadata sync'
  ...
  SELECT
    ...
    a.id,
    'ACTIVE',
    COALESCE(u.effective_from, CURRENT_DATE),
    'system_seed',
    'Migration 0210 - ERP-Lite template apply usage mappings',
    1,
    u.rec_order,
    true
  FROM public.sys_fin_usage_tpl_dtl u
  ...
  IF v_period_tpl_id IS NOT NULL THEN
    ...
    SELECT
      ...
      'system_seed',
      'Migration 0210 - ERP-Lite template apply periods',
      1,
  ...
  LOOP
    ...
    SELECT
      ...
      'system_seed',
      'Migration 0210 - ERP-Lite template apply petty cash',
      1,
...
COMMENT ON FUNCTION public.validate_fin_tpl_for_tnt(UUID, UUID) IS
  'Validate the resolved or explicitly selected ERP-Lite template package for one tenant before manual HQ apply or reapply.';
COMMENT ON FUNCTION public.apply_fin_tpl_for_tnt(UUID, UUID, VARCHAR) IS
  'Explicit HQ-invoked ERP-Lite template materialization function with hardened COA lineage, manual reapply modes, and no tenant-insert auto provisioning.';

-- ============================================================================
-- Migration: 0498_fix_wf_clone_sem_ambiguous_version_no.sql
-- Purpose:   Unblock HQ Studio Clone (Pilot/Published -> new Draft). Postgres
--            42702 (column reference version_no is ambiguous) is raised by
--            sys_wf_prof_ver_clone_sem because RETURNS TABLE (version_id,
--            version_no) creates PL/pgSQL OUT variables that shadow table
--            columns. 0467 already fixed the same class of bug in
--            sys_wf_prof_ver_save_policy; 0494 rewrote clone/delete without
--            that qualification. Same shadowing exists on unused-Draft
--            delete, so both RPCs are replaced here.
--            Also fixes a second, independent clone gap found in review:
--            sys_wf_prof_ver_exec_gate_cf.input_schema_version (added by
--            0462, NOT NULL DEFAULT 1) was missing from the clone's gate
--            insert, so cloning a gate binding authored under a newer
--            evaluator input contract silently downgraded it back to 1 in
--            the new Draft. Now copied verbatim from the source row.
-- Affected:  sys_wf_prof_ver_clone_sem, sys_wf_prof_ver_delete_draft_sem
-- Related:   0462 (input_schema_version added to exec_gate_cf), 0467
--            (save_policy 42702), 0482 (clone create-preset copy), 0494
--            (current clone/delete bodies after artifact retirement)
-- ============================================================================
...
-- ROLLBACK PLAN: restore the 0494 function bodies (unqualified). No rows
...
BEGIN
  ...
  -- Qualify version_id/version_no: RETURNS TABLE OUT vars shadow those names (42702).
  SELECT source_version.*
  INTO v_source
  FROM public.sys_wf_profile_ver_mst AS source_version
  WHERE source_version.profile_id = p_profile_id
    AND source_version.version_no = p_source_version_no
  FOR UPDATE;
  ...
  FOR v_source_exec IN
    ...
    WHERE executable.version_id = v_source.version_id
    ORDER BY executable.display_order, executable.exec_id
  LOOP
    ...
    SELECT
      ...
      execution_gate.input_schema_version,
...
BEGIN
  ...
  FROM public.sys_wf_profile_ver_mst AS source_version
  WHERE source_version.profile_id = p_profile_id
    AND source_version.version_no = p_version_no
  FOR UPDATE;
  ...
  DELETE FROM public.sys_wf_prof_ver_exec_cf AS executable
  WHERE executable.version_id = v_version.version_id;

  DELETE FROM public.sys_wf_prof_ver_mod_st_cf AS module_status
  WHERE module_status.version_id = v_version.version_id;
  ...
  DELETE FROM public.sys_wf_prof_ver_scr_dtl AS screen
  WHERE screen.version_id = v_version.version_id;
...
COMMENT ON FUNCTION public.sys_wf_prof_ver_clone_sem(
  ...
  'Atomically deep-clones every semantic profile policy row into a new revision-one Draft without inheriting artifacts, lifecycle state, graph pins, assignments, or order snapshots. RETURNS TABLE OUT vars are never used as unqualified column references.';

REVOKE ALL ON FUNCTION public.sys_wf_prof_ver_delete_draft_sem(
  UUID, INTEGER, UUID
) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.sys_wf_prof_ver_delete_draft_sem(
  UUID, INTEGER, UUID
) TO service_role;

COMMENT ON FUNCTION public.sys_wf_prof_ver_delete_draft_sem(
  UUID, INTEGER, UUID
) IS
  'Deletes one unused DRAFT semantic profile version and its related policy rows. Pilot/Published/Retired and versions with order snapshots or assignments fail closed. RETURNS TABLE OUT vars are never used as unqualified column references.';

...
DROP FUNCTION IF EXISTS get_user_tenants_jh(UUID) CASCADE;
...
--create get_user_tenants_jh (p_cur_user_id uuid DEFAULT NULL
CREATE OR REPLACE FUNCTION public.get_user_tenants_jh(p_cur_user_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(tenant_id uuid, tenant_name text, tenant_slug text, user_id uuid, org_user_id uuid, user_role text, is_active boolean, last_login_at timestamp without time zone)
AS $$
BEGIN
  RETURN QUERY
  SELECT
...
GRANT EXECUTE ON FUNCTION public.get_user_tenants_jh(uuid) TO PUBLIC;

GRANT EXECUTE ON FUNCTION public.get_user_tenants_jh(uuid) TO anon;

GRANT EXECUTE ON FUNCTION public.get_user_tenants_jh(uuid) TO authenticated;

GRANT EXECUTE ON FUNCTION public.get_user_tenants_jh(uuid) TO postgres;

GRANT EXECUTE ON FUNCTION public.get_user_tenants_jh(uuid) TO service_role;

-- ==================================================================
-- Migration: 0142_preference_resolution_functions.sql
-- Purpose: Add get_last_order_preferences and suggest_preferences_from_history
-- Created: 2026-03-12
...
-- Used for "Repeat Last Order" feature
-- ==================================================================
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      ARRAY_AGG(DISTINCT oisp.preference_code) FILTER (WHERE oisp.preference_code IS NOT NULL),
      '{}'::TEXT[]
    ) AS service_pref_codes
  FROM org_order_items_dtl oi
  LEFT JOIN org_order_item_service_prefs oisp
    ON oisp.order_item_id = oi.id
...
-- Used for "Smart Suggestions" feature
-- ==================================================================
...
BEGIN
  RETURN QUERY
  SELECT
    oisp.preference_code::VARCHAR(50),

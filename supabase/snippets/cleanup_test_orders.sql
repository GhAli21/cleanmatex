--rollback;
-- =============================================================================
-- Cleanup snippet: delete tenant-scoped test orders and related operational data
--
-- Safe defaults:
--   - explicit tenant required
--   - explicit target orders required
--   - dry run by default
...
--
-- Workflow:
--   1. Fill `cleanup_config`
--   2. Add target order ids and/or order numbers
--   3. Run with `execute = false`
--   4. Review the preview result sets
--   5. Flip `execute = true` and re-run
-- =============================================================================
...
-- Uncomment one or both input blocks below.
-- INSERT INTO cleanup_order_ids (order_id) VALUES
--   ('11111111-1111-1111-1111-111111111111'::uuid);
--
-- INSERT INTO cleanup_order_nos (order_no) VALUES
--   ('TEST-ORDER-001');
...
WHERE o.tenant_org_id = cfg.tenant_org_id
  AND (
    (EXISTS (SELECT 1 FROM cleanup_order_ids) AND o.id IN (SELECT order_id FROM cleanup_order_ids))
    OR
    (EXISTS (SELECT 1 FROM cleanup_order_nos) AND o.order_no IN (SELECT order_no FROM cleanup_order_nos))
  );
...
BEGIN
  ...
  IF v_cfg.tenant_org_id = '00000000-0000-0000-0000-000000000000'::uuid THEN
    RAISE EXCEPTION 'Set cleanup_config.tenant_org_id before running this script.';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM cleanup_order_ids)
     AND NOT EXISTS (SELECT 1 FROM cleanup_order_nos) THEN
    RAISE EXCEPTION 'Provide at least one target order id or order number.';
  END IF;

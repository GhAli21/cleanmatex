...
**Operator apply gate:** `0479`–`0488` **applied**. `0488` seeds `WF_V2_HOME_COLLECTION`, **PILOT** + assigned to tenant `c9ac29d1-219c-4a3a-8887-f860550c32be`. 2 bugs found + fixed (tenant-side `semanticEvidenceSchema`/`EvidenceRow` missing `'home_collection'`; `WorkflowActionBar` missing `FAIL_HOME_COLLECTION` reason-prompt) — deployed. Operator also fixed a stuck-at-`intake` gap for this flow via Studio policy edit (`CONFIRM_HOME_COLLECTION.to_status` → `preparing`). **HC1 + HC2 both CLOSED**, confirmed on remote via order-history audit. HQ H1–H3 **done**. Catalog **1.3.0**. Tenant leftover close-out **done**. **Hold floor smoke H1–H4 operator-confirmed done 2026-09-11.** Order details **New order actions** + **Order actions** shipped 2026-09-11. SIMPLE leftover dest **accepted as-is** 2026-09-12 (Studio for later dest edits).

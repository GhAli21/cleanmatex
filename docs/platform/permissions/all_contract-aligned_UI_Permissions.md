...
| `/dashboard/erp-lite/branch-pl` | Branch P&L | `erp_lite_branch_pl:view` | `erp_lite_enabled`, `erp_lite_branch_pl_enabled` | None | None | Live allocation-aware profitability and costing controls route; linked server actions are documented in the API inventory |

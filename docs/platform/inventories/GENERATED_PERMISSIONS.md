# GENERATED Permissions

> **Do not edit by hand.** Regenerate with `npm run rebuild:platform-info-inventories`.


Generated: 2026-07-04T04:18:53.106Z

## By surface (counts)

| Surface | Count |
| --- | --- |
| api | 229 |
| middleware | 4 |
| screen | 19 |
| service | 1 |

## All permission usages

| Permission | Surface | File | Line | Route |
| --- | --- | --- | --- | --- |
| )}]` },
          { status: 403 }
        )
      }

      return authContext
    } catch (error) {
      const message = error instanceof Error ? error.message : | middleware | lib/middleware/require-permission.ts | 165 | lib/middleware/require-permission.ts |
| )}]` },
          { status: 403 }
        )
      }

      return authContext
    } catch (error) {
      const message = error instanceof Error ? error.message : | middleware | lib/middleware/require-permission.ts | 196 | lib/middleware/require-permission.ts |
| )}]`,
            code: | middleware | lib/middleware/tenant-guard.ts | 95 | lib/middleware/tenant-guard.ts |
| )}]`,
            code: | middleware | lib/middleware/tenant-guard.ts | 144 | lib/middleware/tenant-guard.ts |
| admin:read | api | app/api/admin/jwt-health/route.ts | 21 | /api/admin/jwt-health |
| ar_aging:view | api | app/api/v1/ar/reports/aging/route.ts | 12 | /api/v1/ar/reports/aging |
| ar_credits:apply | api | app/api/v1/ar/credits/applications/route.ts | 12 | /api/v1/ar/credits/applications |
| ar_credits:reverse | api | app/api/v1/ar/credits/applications/[id]/reverse/route.ts | 17 | /api/v1/ar/credits/applications/[id]/reverse |
| ar_credits:view | api | app/api/v1/ar/credits/route.ts | 12 | /api/v1/ar/credits |
| ar_disputes:create | api | app/api/v1/ar/disputes/route.ts | 36 | /api/v1/ar/disputes |
| ar_disputes:resolve | api | app/api/v1/ar/disputes/[id]/resolve/route.ts | 17 | /api/v1/ar/disputes/[id]/resolve |
| ar_disputes:view | api | app/api/v1/ar/disputes/route.ts | 15 | /api/v1/ar/disputes |
| ar_dunning:run | api | app/api/v1/ar/dunning/run/route.ts | 12 | /api/v1/ar/dunning/run |
| ar_dunning:view | api | app/api/v1/ar/dunning/route.ts | 12 | /api/v1/ar/dunning |
| ar_ledger:view | api | app/api/v1/ar/customers/[customerId]/balance/route.ts | 16 | /api/v1/ar/customers/[customerId]/balance |
| ar_ledger:view | api | app/api/v1/ar/customers/[customerId]/ledger/route.ts | 17 | /api/v1/ar/customers/[customerId]/ledger |
| ar_stmt_cycles:manage | api | app/api/v1/ar/statement-cycles/route.ts | 39 | /api/v1/ar/statement-cycles |
| ar_stmt_cycles:view | api | app/api/v1/ar/statement-cycles/route.ts | 18 | /api/v1/ar/statement-cycles |
| ar_stmt_cycles:view | api | app/api/v1/ar/statement-cycles/[id]/preview/route.ts | 17 | /api/v1/ar/statement-cycles/[id]/preview |
| b2b_contacts:create | api | app/api/v1/b2b-contacts/route.ts | 60 | /api/v1/b2b-contacts |
| b2b_contacts:create | api | app/api/v1/b2b-contacts/[id]/route.ts | 101 | /api/v1/b2b-contacts/[id] |
| b2b_contacts:create | api | app/api/v1/b2b-contacts/[id]/route.ts | 143 | /api/v1/b2b-contacts/[id] |
| b2b_contacts:view | api | app/api/v1/b2b-contacts/route.ts | 31 | /api/v1/b2b-contacts |
| b2b_contacts:view | api | app/api/v1/b2b-contacts/[id]/route.ts | 38 | /api/v1/b2b-contacts/[id] |
| b2b_contracts:create | api | app/api/v1/b2b-contracts/route.ts | 52 | /api/v1/b2b-contracts |
| b2b_contracts:create | api | app/api/v1/b2b-contracts/[id]/route.ts | 67 | /api/v1/b2b-contracts/[id] |
| b2b_contracts:create | api | app/api/v1/b2b-contracts/[id]/route.ts | 107 | /api/v1/b2b-contracts/[id] |
| b2b_contracts:view | api | app/api/v1/b2b-contracts/route.ts | 29 | /api/v1/b2b-contracts |
| b2b_contracts:view | api | app/api/v1/b2b-contracts/[id]/route.ts | 35 | /api/v1/b2b-contracts/[id] |
| b2b_statements:create | api | app/api/v1/b2b/run-dunning-actions/route.ts | 52 | /api/v1/b2b/run-dunning-actions |
| b2b_statements:create | api | app/api/v1/b2b-statements/generate/route.ts | 25 | /api/v1/b2b-statements/generate |
| b2b_statements:create | api | app/api/v1/b2b-statements/[id]/route.ts | 62 | /api/v1/b2b-statements/[id] |
| b2b_statements:view | api | app/api/v1/b2b/overdue-statements/route.ts | 37 | /api/v1/b2b/overdue-statements |
| b2b_statements:view | api | app/api/v1/b2b-statements/route.ts | 16 | /api/v1/b2b-statements |
| b2b_statements:view | api | app/api/v1/b2b-statements/[id]/print/route.ts | 22 | /api/v1/b2b-statements/[id]/print |
| b2b_statements:view | api | app/api/v1/b2b-statements/[id]/route.ts | 30 | /api/v1/b2b-statements/[id] |
| cash_drawer:close_session | screen | src/features/pos-sessions/ui/pos-sessions-screen.tsx | 96 | src/features/pos-sessions/ui/pos-sessions-screen.tsx |
| cash_drawer:close_session | api | app/api/v1/cash-drawers/[drawerId]/close-session/route.ts | 26 | /api/v1/cash-drawers/[drawerId]/close-session |
| cash_drawer:open_session | api | app/api/v1/cash-drawers/[drawerId]/open-session/route.ts | 25 | /api/v1/cash-drawers/[drawerId]/open-session |
| cash_drawer:record_movement | api | app/api/v1/cash-drawers/[drawerId]/cash-movement/route.ts | 26 | /api/v1/cash-drawers/[drawerId]/cash-movement |
| cash_drawer:view | api | app/api/v1/cash-drawers/route.ts | 10 | /api/v1/cash-drawers |
| cash_drawer:view | api | app/api/v1/cash-drawers/[drawerId]/session/[sessionId]/summary/route.ts | 15 | /api/v1/cash-drawers/[drawerId]/session/[sessionId]/summary |
| config:preferences_manage | api | app/api/v1/catalog/order-sources/route.ts | 23 | /api/v1/catalog/order-sources |
| config:preferences_manage | api | app/api/v1/catalog/order-sources/route.ts | 63 | /api/v1/catalog/order-sources |
| config:preferences_manage | api | app/api/v1/catalog/packing-preferences/admin/route.ts | 21 | /api/v1/catalog/packing-preferences/admin |
| config:preferences_manage | api | app/api/v1/catalog/packing-preferences/route.ts | 51 | /api/v1/catalog/packing-preferences |
| config:preferences_manage | api | app/api/v1/catalog/packing-preferences/[code]/route.ts | 27 | /api/v1/catalog/packing-preferences/[code] |
| config:preferences_manage | api | app/api/v1/catalog/packing-preferences/[code]/route.ts | 90 | /api/v1/catalog/packing-preferences/[code] |
| config:preferences_manage | api | app/api/v1/catalog/preference-bundles/route.ts | 55 | /api/v1/catalog/preference-bundles |
| config:preferences_manage | api | app/api/v1/catalog/preference-bundles/[id]/route.ts | 27 | /api/v1/catalog/preference-bundles/[id] |
| config:preferences_manage | api | app/api/v1/catalog/preference-bundles/[id]/route.ts | 82 | /api/v1/catalog/preference-bundles/[id] |
| config:preferences_manage | api | app/api/v1/catalog/preference-kinds/admin/route.ts | 25 | /api/v1/catalog/preference-kinds/admin |
| config:preferences_manage | api | app/api/v1/catalog/preference-kinds/admin/route.ts | 55 | /api/v1/catalog/preference-kinds/admin |
| config:preferences_manage | api | app/api/v1/catalog/service-preferences/admin/route.ts | 21 | /api/v1/catalog/service-preferences/admin |
| config:preferences_manage | api | app/api/v1/catalog/service-preferences/route.ts | 55 | /api/v1/catalog/service-preferences |
| config:preferences_manage | api | app/api/v1/catalog/service-preferences/[code]/route.ts | 27 | /api/v1/catalog/service-preferences/[code] |
| config:preferences_manage | api | app/api/v1/catalog/service-preferences/[code]/route.ts | 90 | /api/v1/catalog/service-preferences/[code] |
| config:preferences_manage | api | app/api/v1/customer-categories/check-code/route.ts | 22 | /api/v1/customer-categories/check-code |
| config:preferences_manage | api | app/api/v1/customer-categories/route.ts | 58 | /api/v1/customer-categories |
| config:preferences_manage | api | app/api/v1/customer-categories/[code]/route.ts | 74 | /api/v1/customer-categories/[code] |
| config:preferences_manage | api | app/api/v1/customer-categories/[code]/route.ts | 133 | /api/v1/customer-categories/[code] |
| customer_statements:view | api | app/api/v1/ar/customers/[customerId]/statements/print/route.ts | 17 | /api/v1/ar/customers/[customerId]/statements/print |
| customer_statements:view | api | app/api/v1/ar/customers/[customerId]/statements/route.ts | 17 | /api/v1/ar/customers/[customerId]/statements |
| customers:create | api | app/api/v1/customers/route.ts | 57 | /api/v1/customers |
| customers:delete | api | app/api/v1/customers/[id]/route.ts | 214 | /api/v1/customers/[id] |
| customers:read | api | app/api/v1/customer-categories/route.ts | 23 | /api/v1/customer-categories |
| customers:read | api | app/api/v1/customer-categories/[code]/route.ts | 27 | /api/v1/customer-categories/[code] |
| customers:read | api | app/api/v1/customers/[id]/route.ts | 43 | /api/v1/customers/[id] |
| customers:read | api | app/api/v1/customers/[id]/service-prefs/route.ts | 26 | /api/v1/customers/[id]/service-prefs |
| customers:receipt_allocate | screen | src/features/customers/ui/customer-account-receipt-client.tsx | 59 | src/features/customers/ui/customer-account-receipt-client.tsx |
| customers:update | api | app/api/v1/customers/[id]/route.ts | 126 | /api/v1/customers/[id] |
| customers:update | api | app/api/v1/customers/[id]/service-prefs/route.ts | 74 | /api/v1/customers/[id]/service-prefs |
| customers:update | api | app/api/v1/customers/[id]/service-prefs/route.ts | 140 | /api/v1/customers/[id]/service-prefs |
| drivers:read | api | app/api/v1/delivery/routes/route.ts | 16 | /api/v1/delivery/routes |
| fin_voucher_lines:delete_draft | api | app/api/v1/finance/vouchers/[voucherId]/lines/[lineId]/route.ts | 43 | /api/v1/finance/vouchers/[voucherId]/lines/[lineId] |
| fin_voucher_lines:update | api | app/api/v1/finance/vouchers/[voucherId]/lines/[lineId]/route.ts | 17 | /api/v1/finance/vouchers/[voucherId]/lines/[lineId] |
| fin_vouchers:cancel | api | app/api/v1/finance/vouchers/[voucherId]/cancel/route.ts | 15 | /api/v1/finance/vouchers/[voucherId]/cancel |
| fin_vouchers:create | api | app/api/v1/finance/vouchers/route.ts | 49 | /api/v1/finance/vouchers |
| fin_vouchers:post | api | app/api/v1/finance/vouchers/[voucherId]/post/route.ts | 18 | /api/v1/finance/vouchers/[voucherId]/post |
| fin_vouchers:reverse | api | app/api/v1/finance/vouchers/[voucherId]/reverse/route.ts | 15 | /api/v1/finance/vouchers/[voucherId]/reverse |
| fin_vouchers:update | api | app/api/v1/finance/vouchers/[voucherId]/route.ts | 42 | /api/v1/finance/vouchers/[voucherId] |
| fin_vouchers:view | api | app/api/v1/finance/vouchers/lookups/expense-categories/route.ts | 11 | /api/v1/finance/vouchers/lookups/expense-categories |
| fin_vouchers:view | api | app/api/v1/finance/vouchers/lookups/line-roles/route.ts | 11 | /api/v1/finance/vouchers/lookups/line-roles |
| fin_vouchers:view | api | app/api/v1/finance/vouchers/lookups/types/route.ts | 11 | /api/v1/finance/vouchers/lookups/types |
| fin_vouchers:view | api | app/api/v1/finance/vouchers/route.ts | 16 | /api/v1/finance/vouchers |
| fin_vouchers:view | api | app/api/v1/finance/vouchers/[voucherId]/route.ts | 17 | /api/v1/finance/vouchers/[voucherId] |
| fin_vouchers:view_effects | api | app/api/v1/finance/voucher-lines/[lineId]/linked-effects/route.ts | 15 | /api/v1/finance/voucher-lines/[lineId]/linked-effects |
| fin_vouchers:view_effects | api | app/api/v1/finance/vouchers/[voucherId]/linked-effects/route.ts | 15 | /api/v1/finance/vouchers/[voucherId]/linked-effects |
| finance_reports:view | api | app/api/v1/finance/reports/money-position/route.ts | 16 | /api/v1/finance/reports/money-position |
| finance_reports:view | api | app/api/v1/finance/reports/orders-summary/route.ts | 16 | /api/v1/finance/reports/orders-summary |
| finance_reports:view | api | app/api/v1/finance/reports/payments-breakdown/route.ts | 11 | /api/v1/finance/reports/payments-breakdown |
| finance_reports:view | api | app/api/v1/finance/reports/reconciliation/b2b-statements/route.ts | 17 | /api/v1/finance/reports/reconciliation/b2b-statements |
| finance_reports:view | api | app/api/v1/finance/reports/reconciliation/cash-drawer/route.ts | 18 | /api/v1/finance/reports/reconciliation/cash-drawer |
| finance_reports:view | api | app/api/v1/finance/reports/reconciliation/excess-liability/route.ts | 18 | /api/v1/finance/reports/reconciliation/excess-liability |
| finance_reports:view | api | app/api/v1/finance/reports/reconciliation/overpayment-disposition/route.ts | 18 | /api/v1/finance/reports/reconciliation/overpayment-disposition |
| finance_reports:view | api | app/api/v1/finance/reports/tax-report/route.ts | 11 | /api/v1/finance/reports/tax-report |
| gift_cards:view | api | app/api/v1/gift-cards/[cardCode]/balance/route.ts | 15 | /api/v1/gift-cards/[cardCode]/balance |
| gift_cards:view | api | app/api/v1/gift-cards/[cardCode]/ledger/route.ts | 15 | /api/v1/gift-cards/[cardCode]/ledger |
| invoices:allocate_payment | screen | src/features/ar/ui/ar-invoice-detail-actions.tsx | 69 | src/features/ar/ui/ar-invoice-detail-actions.tsx |
| invoices:allocate_payment | api | app/api/v1/ar/invoices/[id]/allocations/route.ts | 17 | /api/v1/ar/invoices/[id]/allocations |
| invoices:allocate_payment | api | app/api/v1/ar/invoices/[id]/allocations/[allocationId]/reverse/route.ts | 17 | /api/v1/ar/invoices/[id]/allocations/[allocationId]/reverse |
| invoices:approve_sensitive | screen | src/features/ar/ui/ar-invoice-detail-actions.tsx | 68 | src/features/ar/ui/ar-invoice-detail-actions.tsx |
| invoices:approve_sensitive | api | app/api/v1/ar/invoices/[id]/approve-sensitive/route.ts | 17 | /api/v1/ar/invoices/[id]/approve-sensitive |
| invoices:create | api | app/api/v1/ar/invoices/from-orders/route.ts | 12 | /api/v1/ar/invoices/from-orders |
| invoices:create | api | app/api/v1/ar/invoices/route.ts | 39 | /api/v1/ar/invoices |
| invoices:credit_note | screen | src/features/ar/ui/ar-invoice-detail-actions.tsx | 70 | src/features/ar/ui/ar-invoice-detail-actions.tsx |
| invoices:credit_note | api | app/api/v1/ar/invoices/[id]/credit-note/route.ts | 17 | /api/v1/ar/invoices/[id]/credit-note |
| invoices:debit_note | screen | src/features/ar/ui/ar-invoice-detail-actions.tsx | 71 | src/features/ar/ui/ar-invoice-detail-actions.tsx |
| invoices:debit_note | api | app/api/v1/ar/invoices/[id]/debit-note/route.ts | 17 | /api/v1/ar/invoices/[id]/debit-note |
| invoices:export | api | app/api/v1/ar/invoices/export/route.ts | 12 | /api/v1/ar/invoices/export |
| invoices:issue | screen | src/features/ar/ui/ar-invoice-detail-actions.tsx | 67 | src/features/ar/ui/ar-invoice-detail-actions.tsx |
| invoices:issue | api | app/api/v1/ar/invoices/[id]/issue/route.ts | 17 | /api/v1/ar/invoices/[id]/issue |
| invoices:print | screen | src/features/ar/ui/ar-invoice-detail-actions.tsx | 74 | src/features/ar/ui/ar-invoice-detail-actions.tsx |
| invoices:print | api | app/api/v1/ar/invoices/[id]/print/route.ts | 16 | /api/v1/ar/invoices/[id]/print |
| invoices:read | api | app/api/v1/ar/invoices/route.ts | 18 | /api/v1/ar/invoices |
| invoices:read | api | app/api/v1/ar/invoices/[id]/route.ts | 20 | /api/v1/ar/invoices/[id] |
| invoices:update | screen | src/features/ar/ui/ar-invoice-detail-actions.tsx | 66 | src/features/ar/ui/ar-invoice-detail-actions.tsx |
| invoices:update | api | app/api/v1/ar/invoices/[id]/route.ts | 42 | /api/v1/ar/invoices/[id] |
| invoices:void | screen | src/features/ar/ui/ar-invoice-detail-actions.tsx | 73 | src/features/ar/ui/ar-invoice-detail-actions.tsx |
| invoices:void | api | app/api/v1/ar/invoices/[id]/void/route.ts | 17 | /api/v1/ar/invoices/[id]/void |
| invoices:write_off | screen | src/features/ar/ui/ar-invoice-detail-actions.tsx | 72 | src/features/ar/ui/ar-invoice-detail-actions.tsx |
| invoices:write_off | api | app/api/v1/ar/invoices/[id]/write-off/route.ts | 17 | /api/v1/ar/invoices/[id]/write-off |
| loyalty:manage_config | api | app/api/v1/loyalty/config/route.ts | 43 | /api/v1/loyalty/config |
| loyalty:manage_config | api | app/api/v1/loyalty/tiers/route.ts | 24 | /api/v1/loyalty/tiers |
| loyalty:view | api | app/api/v1/customers/[id]/loyalty/route.ts | 15 | /api/v1/customers/[id]/loyalty |
| loyalty:view_config | api | app/api/v1/loyalty/config/route.ts | 14 | /api/v1/loyalty/config |
| notifications:configure | api | app/api/v1/notifications/campaigns/[id]/status/route.ts | 121 | /api/v1/notifications/campaigns/[id]/status |
| notifications:configure | api | app/api/v1/notifications/settings/providers/route.ts | 32 | /api/v1/notifications/settings/providers |
| notifications:configure | api | app/api/v1/notifications/settings/providers/route.ts | 70 | /api/v1/notifications/settings/providers |
| notifications:configure | api | app/api/v1/notifications/settings/providers/route.ts | 133 | /api/v1/notifications/settings/providers |
| notifications:configure | api | app/api/v1/notifications/settings/providers/route.ts | 209 | /api/v1/notifications/settings/providers |
| notifications:configure | api | app/api/v1/notifications/settings/route.ts | 21 | /api/v1/notifications/settings |
| notifications:configure | api | app/api/v1/notifications/settings/route.ts | 53 | /api/v1/notifications/settings |
| notifications:manage | api | app/api/v1/notifications/campaigns/route.ts | 38 | /api/v1/notifications/campaigns |
| notifications:manage | api | app/api/v1/notifications/campaigns/route.ts | 96 | /api/v1/notifications/campaigns |
| notifications:manage | api | app/api/v1/notifications/campaigns/[id]/route.ts | 24 | /api/v1/notifications/campaigns/[id] |
| notifications:manage | api | app/api/v1/notifications/campaigns/[id]/status/route.ts | 61 | /api/v1/notifications/campaigns/[id]/status |
| notifications:manage | api | app/api/v1/notifications/campaigns/[id]/test/route.ts | 29 | /api/v1/notifications/campaigns/[id]/test |
| notifications:manage | api | app/api/v1/notifications/read-all/route.ts | 16 | /api/v1/notifications/read-all |
| notifications:manage | api | app/api/v1/notifications/user-prefs/route.ts | 17 | /api/v1/notifications/user-prefs |
| notifications:manage | api | app/api/v1/notifications/user-prefs/route.ts | 44 | /api/v1/notifications/user-prefs |
| notifications:manage | api | app/api/v1/notifications/[id]/read/route.ts | 22 | /api/v1/notifications/[id]/read |
| notifications:read | api | app/api/notifications/push-subscription/route.ts | 47 | /api/notifications/push-subscription |
| notifications:read | api | app/api/notifications/push-subscription/route.ts | 108 | /api/notifications/push-subscription |
| notifications:read | api | app/api/v1/notifications/route.ts | 19 | /api/v1/notifications |
| notifications:read | api | app/api/v1/notifications/unread-count/route.ts | 19 | /api/v1/notifications/unread-count |
| notifications:view_log | api | app/api/v1/notifications/delivery-log/route.ts | 19 | /api/v1/notifications/delivery-log |
| orders:apply_credit | api | app/api/v1/orders/[id]/credit-applications/route.ts | 35 | /api/v1/orders/[id]/credit-applications |
| orders:approve_refund | api | app/api/v1/orders/refunds/[refundId]/approve/route.ts | 19 | /api/v1/orders/refunds/[refundId]/approve |
| orders:collect_payment | screen | src/features/orders/ui/collect-payment/order-collect-payment-modal.tsx | 123 | src/features/orders/ui/collect-payment/order-collect-payment-modal.tsx |
| orders:collect_payment | api | app/api/v1/orders/[id]/collect-payment/route.ts | 36 | /api/v1/orders/[id]/collect-payment |
| orders:collect_payment | api | app/api/v1/orders/[id]/payments/route.ts | 40 | /api/v1/orders/[id]/payments |
| orders:create | screen | src/features/auth/ui/RequirePermission.tsx | 35 | src/features/auth/ui/RequirePermission.tsx |
| orders:create | api | app/api/v1/orders/preview-financials/route.ts | 22 | /api/v1/orders/preview-financials |
| orders:create | api | app/api/v1/orders/preview-payment/route.ts | 26 | /api/v1/orders/preview-payment |
| orders:create | api | app/api/v1/orders/route.ts | 34 | /api/v1/orders |
| orders:create | api | app/api/v1/orders/submit-order/route.ts | 72 | /api/v1/orders/submit-order |
| orders:create | api | app/api/v1/orders/[id]/items/[itemId]/pieces/route.ts | 94 | /api/v1/orders/[id]/items/[itemId]/pieces |
| orders:create_adjustment | api | app/api/v1/orders/[id]/adjustments/route.ts | 38 | /api/v1/orders/[id]/adjustments |
| orders:delete | api | app/api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/route.ts | 193 | /api/v1/orders/[id]/items/[itemId]/pieces/[pieceId] |
| orders:overpayment_allocate | api | app/api/v1/customers/[id]/open-balances/route.ts | 15 | /api/v1/customers/[id]/open-balances |
| orders:process_refund | api | app/api/v1/orders/refunds/[refundId]/process/route.ts | 19 | /api/v1/orders/refunds/[refundId]/process |
| orders:process_refund | api | app/api/v1/orders/[id]/refund/route.ts | 47 | /api/v1/orders/[id]/refund |
| orders:process_refund | api | app/api/v1/orders/[id]/refunds/route.ts | 89 | /api/v1/orders/[id]/refunds |
| orders:read | api | app/api/v1/catalog/packing-preferences/route.ts | 22 | /api/v1/catalog/packing-preferences |
| orders:read | api | app/api/v1/catalog/preference-bundles/route.ts | 22 | /api/v1/catalog/preference-bundles |
| orders:read | api | app/api/v1/catalog/preference-kinds/route.ts | 21 | /api/v1/catalog/preference-kinds |
| orders:read | api | app/api/v1/catalog/service-preferences/route.ts | 22 | /api/v1/catalog/service-preferences |
| orders:read | api | app/api/v1/orders/pieces/[pieceId]/history/route.ts | 36 | /api/v1/orders/pieces/[pieceId]/history |
| orders:read | api | app/api/v1/orders/route.ts | 192 | /api/v1/orders |
| orders:read | api | app/api/v1/orders/[id]/items/[itemId]/pieces/route.ts | 27 | /api/v1/orders/[id]/items/[itemId]/pieces |
| orders:read | api | app/api/v1/orders/[id]/items/[itemId]/pieces/scan/route.ts | 24 | /api/v1/orders/[id]/items/[itemId]/pieces/scan |
| orders:read | api | app/api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/route.ts | 27 | /api/v1/orders/[id]/items/[itemId]/pieces/[pieceId] |
| orders:read | api | app/api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/service-prefs/route.ts | 30 | /api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/service-prefs |
| orders:read | api | app/api/v1/orders/[id]/items/[itemId]/service-prefs/route.ts | 30 | /api/v1/orders/[id]/items/[itemId]/service-prefs |
| orders:read | api | app/api/v1/orders/[id]/pieces/route.ts | 29 | /api/v1/orders/[id]/pieces |
| orders:read | api | app/api/v1/orders/[id]/report/invoices-payments-rprt/route.ts | 53 | /api/v1/orders/[id]/report/invoices-payments-rprt |
| orders:read | api | app/api/v1/orders/[id]/report/payments-rprt/route.ts | 38 | /api/v1/orders/[id]/report/payments-rprt |
| orders:read | api | app/api/v1/orders/[id]/workflow-context/route.ts | 20 | /api/v1/orders/[id]/workflow-context |
| orders:read | api | app/api/v1/preferences/last-order/route.ts | 22 | /api/v1/preferences/last-order |
| orders:read | api | app/api/v1/preferences/resolve/route.ts | 21 | /api/v1/preferences/resolve |
| orders:read | api | app/api/v1/preferences/suggest/route.ts | 22 | /api/v1/preferences/suggest |
| orders:transition | api | app/api/v1/orders/[id]/confirm-physical-intake/route.ts | 24 | /api/v1/orders/[id]/confirm-physical-intake |
| orders:transition | api | app/api/v1/orders/[id]/transition/route.ts | 37 | /api/v1/orders/[id]/transition |
| orders:transition | api | app/api/v1/orders/[id]/transition/route.ts | 175 | /api/v1/orders/[id]/transition |
| orders:update | api | app/api/v1/orders/[id]/batch-update/route.ts | 44 | /api/v1/orders/[id]/batch-update |
| orders:update | api | app/api/v1/orders/[id]/items/[itemId]/apply-bundle/[bundleCode]/route.ts | 30 | /api/v1/orders/[id]/items/[itemId]/apply-bundle/[bundleCode] |
| orders:update | api | app/api/v1/orders/[id]/items/[itemId]/packing-pref/route.ts | 27 | /api/v1/orders/[id]/items/[itemId]/packing-pref |
| orders:update | api | app/api/v1/orders/[id]/items/[itemId]/pieces/route.ts | 192 | /api/v1/orders/[id]/items/[itemId]/pieces |
| orders:update | api | app/api/v1/orders/[id]/items/[itemId]/pieces/sync/route.ts | 27 | /api/v1/orders/[id]/items/[itemId]/pieces/sync |
| orders:update | api | app/api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/conditions/route.ts | 31 | /api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/conditions |
| orders:update | api | app/api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/route.ts | 102 | /api/v1/orders/[id]/items/[itemId]/pieces/[pieceId] |
| orders:update | api | app/api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/service-prefs/confirm/route.ts | 26 | /api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/service-prefs/confirm |
| orders:update | api | app/api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/service-prefs/route.ts | 97 | /api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/service-prefs |
| orders:update | api | app/api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/service-prefs/route.ts | 155 | /api/v1/orders/[id]/items/[itemId]/pieces/[pieceId]/service-prefs |
| orders:update | api | app/api/v1/orders/[id]/items/[itemId]/service-prefs/route.ts | 82 | /api/v1/orders/[id]/items/[itemId]/service-prefs |
| orders:update | api | app/api/v1/orders/[id]/items/[itemId]/service-prefs/route.ts | 139 | /api/v1/orders/[id]/items/[itemId]/service-prefs |
| orders:update | api | app/api/v1/orders/[id]/update/route.ts | 37 | /api/v1/orders/[id]/update |
| orders:verify_payment | screen | src/features/orders/ui/order-financial/order-payments-credits-tables.tsx | 64 | src/features/orders/ui/order-financial/order-payments-credits-tables.tsx |
| orders:verify_payment | service | lib/services/order-settlement.service.ts | 445 | lib/services/order-settlement.service.ts |
| orders:verify_payment | api | app/api/v1/orders/[id]/payments/[paymentId]/verify/route.ts | 42 | /api/v1/orders/[id]/payments/[paymentId]/verify |
| orders:view_financial_breakdown | api | app/api/v1/orders/[id]/financial-summary/route.ts | 15 | /api/v1/orders/[id]/financial-summary |
| orders:view_financial_breakdown | api | app/api/v1/orders/[id]/refunds/route.ts | 54 | /api/v1/orders/[id]/refunds |
| payment_config:manage | api | app/api/v1/settings/payments/card-brands/[brandId]/route.ts | 39 | /api/v1/settings/payments/card-brands/[brandId] |
| payment_config:manage | api | app/api/v1/settings/payments/methods/[methodId]/route.ts | 32 | /api/v1/settings/payments/methods/[methodId] |
| payment_config:manage | api | app/api/v1/settings/payments/terminals/route.ts | 57 | /api/v1/settings/payments/terminals |
| payment_config:view | api | app/api/v1/settings/payments/card-brands/route.ts | 16 | /api/v1/settings/payments/card-brands |
| payment_config:view | api | app/api/v1/settings/payments/methods/route.ts | 12 | /api/v1/settings/payments/methods |
| payment_config:view | api | app/api/v1/settings/payments/terminals/route.ts | 13 | /api/v1/settings/payments/terminals |
| pos_session:close | screen | src/features/pos-sessions/ui/pos-sessions-screen.tsx | 94 | src/features/pos-sessions/ui/pos-sessions-screen.tsx |
| pos_session:close | api | app/api/v1/pos-sessions/close/route.ts | 12 | /api/v1/pos-sessions/close |
| pos_session:force_close | screen | src/features/pos-sessions/ui/pos-sessions-screen.tsx | 95 | src/features/pos-sessions/ui/pos-sessions-screen.tsx |
| pos_session:force_close | api | app/api/v1/pos-sessions/force-close/route.ts | 12 | /api/v1/pos-sessions/force-close |
| pos_session:open | screen | src/features/pos-sessions/ui/pos-sessions-screen.tsx | 92 | src/features/pos-sessions/ui/pos-sessions-screen.tsx |
| pos_session:open | api | app/api/v1/pos-sessions/ensure-for-order-entry/route.ts | 12 | /api/v1/pos-sessions/ensure-for-order-entry |
| pos_session:open | api | app/api/v1/pos-sessions/open/route.ts | 12 | /api/v1/pos-sessions/open |
| pos_session:pause_resume | screen | src/features/pos-sessions/ui/pos-sessions-screen.tsx | 93 | src/features/pos-sessions/ui/pos-sessions-screen.tsx |
| pos_session:pause_resume | api | app/api/v1/pos-sessions/pause/route.ts | 12 | /api/v1/pos-sessions/pause |
| pos_session:pause_resume | api | app/api/v1/pos-sessions/resume/route.ts | 12 | /api/v1/pos-sessions/resume |
| pos_session:view | api | app/api/v1/pos-sessions/my-active/route.ts | 8 | /api/v1/pos-sessions/my-active |
| pos_session:view | api | app/api/v1/pos-sessions/route.ts | 9 | /api/v1/pos-sessions |
| pos_session:view | api | app/api/v1/pos-sessions/[sessionId]/summary/route.ts | 11 | /api/v1/pos-sessions/[sessionId]/summary |
| pos_session:view_all | screen | src/features/pos-sessions/ui/pos-sessions-screen.tsx | 91 | src/features/pos-sessions/ui/pos-sessions-screen.tsx |
| promotions:manage | api | app/api/v1/marketing/promotions/route.ts | 50 | /api/v1/marketing/promotions |
| promotions:manage | api | app/api/v1/marketing/promotions/[promoId]/route.ts | 61 | /api/v1/marketing/promotions/[promoId] |
| promotions:manage | api | app/api/v1/marketing/promotions/[promoId]/route.ts | 109 | /api/v1/marketing/promotions/[promoId] |
| promotions:view | api | app/api/v1/marketing/promotions/route.ts | 12 | /api/v1/marketing/promotions |
| promotions:view | api | app/api/v1/marketing/promotions/validate/route.ts | 21 | /api/v1/marketing/promotions/validate |
| promotions:view | api | app/api/v1/marketing/promotions/[promoId]/route.ts | 19 | /api/v1/marketing/promotions/[promoId] |
| reconciliation:acknowledge_issues | api | app/api/v1/finance/reconciliation/issues/[issueId]/route.ts | 29 | /api/v1/finance/reconciliation/issues/[issueId] |
| reconciliation:run | api | app/api/v1/finance/reconciliation/runs/route.ts | 43 | /api/v1/finance/reconciliation/runs |
| reconciliation:run | api | app/api/v1/orders/[id]/financial-reconcile/route.ts | 38 | /api/v1/orders/[id]/financial-reconcile |
| reconciliation:view | api | app/api/v1/finance/reconciliation/runs/route.ts | 12 | /api/v1/finance/reconciliation/runs |
| reconciliation:view | api | app/api/v1/finance/reconciliation/runs/[runId]/route.ts | 46 | /api/v1/finance/reconciliation/runs/[runId] |
| reconciliation:view | api | app/api/v1/finance/vouchers/[voucherId]/reconciliation/route.ts | 37 | /api/v1/finance/vouchers/[voucherId]/reconciliation |
| reconciliation:view | api | app/api/v1/orders/[id]/financial-reconciliation/route.ts | 31 | /api/v1/orders/[id]/financial-reconciliation |
| settings:branding | api | app/api/v1/settings/branding/logo/route.ts | 32 | /api/v1/settings/branding/logo |
| settings:branding | api | app/api/v1/settings/branding/route.ts | 75 | /api/v1/settings/branding |
| settings:organization | api | app/api/v1/settings/general/route.ts | 115 | /api/v1/settings/general |
| settings:read | api | app/api/v1/settings/branding/route.ts | 45 | /api/v1/settings/branding |
| settings:read | api | app/api/v1/settings/general/route.ts | 83 | /api/v1/settings/general |
| stored_value:issue_advance | api | app/api/v1/customers/[id]/advance/issue/route.ts | 26 | /api/v1/customers/[id]/advance/issue |
| stored_value:issue_credit_note | api | app/api/v1/customers/[id]/credit-note/issue/route.ts | 28 | /api/v1/customers/[id]/credit-note/issue |
| stored_value:top_up_wallet | api | app/api/v1/customers/[id]/wallet/top-up/route.ts | 26 | /api/v1/customers/[id]/wallet/top-up |
| stored_value:view | api | app/api/v1/customers/[id]/advance/ledger/route.ts | 16 | /api/v1/customers/[id]/advance/ledger |
| stored_value:view | api | app/api/v1/customers/[id]/credit-notes/route.ts | 15 | /api/v1/customers/[id]/credit-notes |
| stored_value:view | api | app/api/v1/customers/[id]/wallet/ledger/route.ts | 16 | /api/v1/customers/[id]/wallet/ledger |
| stored_value:view_balances | api | app/api/v1/customers/[id]/stored-value/route.ts | 15 | /api/v1/customers/[id]/stored-value |
| tax:manage_config | api | app/api/v1/settings/tax/exemptions/route.ts | 55 | /api/v1/settings/tax/exemptions |
| tax:manage_config | api | app/api/v1/settings/tax/profiles/route.ts | 51 | /api/v1/settings/tax/profiles |
| tax:manage_config | api | app/api/v1/settings/tax/profiles/[profileId]/route.ts | 29 | /api/v1/settings/tax/profiles/[profileId] |
| tax:view_config | api | app/api/v1/settings/tax/exemptions/route.ts | 13 | /api/v1/settings/tax/exemptions |
| tax:view_config | api | app/api/v1/settings/tax/profiles/route.ts | 13 | /api/v1/settings/tax/profiles |

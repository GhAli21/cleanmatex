...
- 5-step dropdown (sorting, pretreatment, washing, drying, finishing): this is an optional list without any effect just informative.
- Done checkbox
- Split checkbox
...
- [x] Create workflow template tables (sys_workflow_template_cd, stages, transitions) with 5 default templates
- [x] Create tenant workflow configuration tables (settings, service category overrides)
- [x] Extend org_orders_mst and org_order_items_dtl with workflow fields
- [x] Create org_order_item_issues and org_order_item_processing_steps tables
- [x] Create comprehensive org_order_history table with action types
- [x] Implement cmx_order_transition() PostgreSQL function with validation and history logging
- [x] Enhance WorkflowService with template resolution, transition validation, quality gates
- [x] Create OrderService with createOrder, estimateReadyBy, splitOrder, issue management
- [x] Create ItemProcessingService for 5-step tracking and item completion
- [x] Implement order API routes (create, state, transition, split, issue, history, estimate-ready-by)
- [x] Implement item processing API routes (record step, mark complete)
- [x] Build New Order page with category tabs, product grid, summary panel matching UI samples
- [x] Create New Order components (category tabs, product grid, summary panel, customer picker)
- [x] Build Preparation list and detail pages for Quick Drop itemization
- [x] Build Processing list and detail pages with 5-step tracking, split, rack location
- [x] Build Assembly list and detail pages (conditional on tenant settings)
- [x] Build QA list and detail pages with accept/reject/solve actions
- [x] Build Ready list and detail pages with deliver and return-to-processing actions
- [x] Enhance order detail page with workflow timeline, issue log, split order links
- [x] Create order history timeline component displaying all action types
- [x] Add TypeScript types for workflow templates, stages, transitions, issues, steps
- [x] Create Zod validation schemas for all workflow API requests
- [x] Add EN/AR translations for all workflow screens, statuses, and actions
- [x] Define roles and implement role-based access guards for all screens
- [x] Add role validation to transition API endpoints
- [x] Write database tests for workflow functions and quality gates
- [x] Write unit tests for WorkflowService, OrderService, ItemProcessingService
- [x] Write integration tests for all order workflow API endpoints
- [x] Write E2E tests for complete order workflows (normal, quick drop, split, QA rejection)
- [x] Create data migration script to convert existing orders to new workflow structure
- [x] Remove old order implementation and deprecated code
- [x] Update navigation menu with new workflow screens (Preparation, Processing, Assembly, QA, Ready)
...
|-----|------|----------------|
| 0344 | `0344_notif_catalog_schema.sql` | `sys_notif_categories_cd`, `sys_notification_events_cd`, `sys_notif_event_channel_defaults`. Adds `WEB_SOCKET` to `sys_notification_channel_cd`. |
...
| 0351 | `0351_notif_push_subscriptions.sql` | `org_notif_push_subs_dtl` (provider-agnostic push subscription registry). Seeds 4 additional providers: RESEND, VAPID, ONESIGNAL, META_WHATSAPP. |
...
SELECT code, name FROM sys_notification_channel_cd ORDER BY code;
...
WHERE table_schema = 'public'
  AND table_name IN (
    ...
    'org_notif_push_subs_dtl', 'org_ntf_channel_provider_cf'
  )
ORDER BY table_name;
...
4. Remove the `WEB_SOCKET` row from `sys_notification_channel_cd`

...
- [ ] Add a new drawer → appears in list
...
- [ ] Create a new tax profile: name, VAT rate (e.g. 15%), type = VAT → saves
- [ ] Set it as the default → saved flag reflected in list
- [ ] Add a tax exemption for a customer category → saves

---

## 3. Cash Drawer Lifecycle `/dashboard/billing/cash-drawers`

- [ ] List page loads all drawers
- [ ] Open a session: click drawer → Open Session → enter opening float (e.g. 500)
...
- [ ] Close session: enter closing float → session status → CLOSED
...
**Single CASH payment:**
- [ ] Add items → payment modal shows correct subtotal + VAT
- [ ] Select CASH, enter amount → order created successfully
- [ ] Navigate to `billing/payments/[id]/print/receipt-voucher` → receipt renders with tax breakdown

**Split payment (CASH + CARD):**
- [ ] Add items → in payment modal, split into two legs: CASH 50 + CARD 50 (or whatever total is)
- [ ] Both legs submitted → order created, no errors

**PAY_ON_COLLECTION:**
- [ ] Select PAY_ON_COLLECTION → order created with status `PENDING_COLLECTION`
- [ ] Go to order detail → Collect Payment → enter CASH amount → payment recorded
...
- [ ] List loads
- [ ] Create a promotion: type = PERCENTAGE, value = 10%, code = TEST10 → saves
- [ ] Go to New Order → enter promo code TEST10 → discount applied in preview
...
- [ ] After creating a paid order → check customer record for loyalty points earned (async via outbox — may need a moment)
...
- [ ] Select a customer → Stored Value tab → wallet balance shows
- [ ] Top-up wallet: enter 200 → balance updates
- [ ] Create new order → apply wallet credit in payment modal → deducted correctly from wallet

---

## 8. Refunds `/dashboard/billing/refunds`

- [ ] Refunds list page loads
- [ ] On a completed paid order → initiate refund: select method (CASH), amount ≤ paid amount
- [ ] Refund appears in list with status PENDING
- [ ] Approve refund → status → APPROVED
- [ ] Process refund → status → PROCESSED

---

## 9. Reconciliation `/dashboard/billing/reconciliation`
...
- [ ] Run appears in list
- [ ] Click run → detail page shows issues (BLOCKER / WARNING / INFO) or "no issues"
- [ ] Export CSV button → downloads file with correct columns
...
- [ ] Export CSV on any tab → file downloads correctly

---

## 11. Order Financial Tab

- [ ] Open any completed order → `orders/[id]/full` page
- [ ] Financial tab visible → shows charges, taxes, discounts, payment legs breakdown
...
**Start with flows 1 → 2 → 3 → 4** in that order — each depends on the previous being configured. The rest can be tested in any order.

...
- **Phase 4 — Presets + view renderer (strangler decomposition). 🟡 IN PROGRESS.** 4a ✅ pure L6 preset descriptors (`SIMPLE_PRESET`/`FULL_PRESET` + `PaymentPreset` type) + `resolvePreset` + `resolveViewPresentation` merge (unavailable/blocked/required invariants), tested 10/10. 4b ✅ pure L5 view-plan core (`planCapabilityView` + inline/dialog/blocked/required selectors, `payment/view/capability-view-plan.ts`), tested 7/7. 4c ✅ React `CapabilityViewRenderer` (`payment/view/capability-view-renderer.tsx`) — 3 regions (inline via `renderInline` delegation, dialog-opener buttons via `onOpenCapability`, deduped `PaymentSubmitGuard` banners via `selectGuardSlots`), no finance/props knowledge, behind kill-switch, tested 5/5. 4d ✅ pure engine→context projector (`payment/domain/project-capability-context.ts`: `projectCapabilityContext` + `CapabilityContextSource`) — encodes method-classification / gift-promo-flag fold / precise CREDIT_INVOICE policy / FX-rounding rules, container-derived facts pass through; tested 10/10 incl. end-to-end projector→registry. **4e ✅ FIRST real container wiring** — FX/rounding inline line routed through `CapabilityViewRenderer` in `payment-full-view.tsx` right rail behind `PAYMENT_MODE_USER_CONTROLLED` (additive: FX line was never rendered; double-gated; only FX routed so far via `.filter(FX_ROUNDING)`). Gates incl. `npm run build` ✓. **4f ✅ hardening #5** — `deriveSimpleModeMethodOptions` re-expressed as preset metadata: `applyMethodChipPolicy(options, policy)` (`payment/view/method-chips.ts`, reuses the single-source deriver for `simpleSafeOnly`); container `simpleMethodOptions` now preset-driven (`SIMPLE_PRESET.methodChips`), behavior-identical (parity test 4/4). State-survival (hardening #4) is architecturally guaranteed (no engine renderHook harness exists) → manual-QA item. Next: 4g route an inline section's show/hide decision through the registry (behavior freeze) → then Phase 5 behavior reversal. `SIMPLE` + `FULL` presets over the renderer; Full view routed through capabilities **section by section**, oracle green at each step (no rewrite); `deriveSimpleModeMethodOptions` re-expressed as preset metadata (hardening #5); type scaffolding for future presets. State-survival invariant enforced + tested (hardening #4).
- **Phase 5 — Behavior reversal + error routing. 🟡 IN PROGRESS.** Server-error→capability-guard routing ✅ (`routeServerErrorToGuard`, pure). **Behavior reversal WIRED ✅ (2026-07-10):** kill-switch now consumed via `resolvePaymentModalConfig().userControlledMode`; auto-escalation + Simple-lock + handleModeChange-refusal all gated off by default; `autoEscalated` banner replaced by the dismissible `PaymentModeSuggestion` primitive (5/5 tests); gates green incl. `next build`. Remaining: wire `routeServerErrorToGuard` into `use-order-submission.ts:607-646`; manual QA. Original scope: Remove auto-escalation/lock (per inventory); user-controlled mode; dismissible suggestion; #8 guard; #3 B2B required dialog; restricted-class guards with reason codes. Extend the `infrastructureMessages` map into **server-error → capability-guard routing** so rejections render in the active view without forcing Full (hardening #2). Error boundary around dialogs (hardening #11); observability hooks (hardening #12).

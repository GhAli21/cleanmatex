...
| 2.1c | Overpayment in-place *(new 2026-07-10)* | Overpay (pay-extra ON) in Simple → click Route extra amount | Extra-receipt routing dialog opens **over Simple**; choosing a destination works exactly as from Advanced | [ yes] |

| 2.2 | In-place dialogs | Click each button | Its dialog opens **over Simple**; closing returns to Simple with state intact | [ yes] | 
...
| 2.6 | Dialog cancel/Done | Cancel and Done from each dialog | Engine state never lost either way | [ yes] | some windows no need for Cancel button 
...
| 4.2 | B2B credit exceeded | B2B customer, bill-to-account beyond limit → **Submit** | Footer red **server guard** + corrective **"Account billing"** opens the B2B dialog in-place (both faces). *Not* the right-rail card alone — that card’s **"Review account billing"** also opens the same dialog (client-side), but §4.2 is the post-Submit guard. Hard-deny: dialog does **not** unlock override. | [ yes ] |

| 4.3 | Guard lifecycle | After a guard shows: fix the issue, Submit again | Guard clears on the new attempt; does not reappear after close/reopen | [ yes] | 

| 4.4 | Both faces | Trigger a guard, flip Simple↔Advanced | Banner visible in both (shared footer) | [ yes] |

| 4.5 | Generic errors | Force a non-typed failure (e.g. network drop) | Toast only — no guard, no view switch | [ yes] | 
...
| 5.4 | Narrow / tablet | Window below ~1280px | Receipt rail becomes slide-over; docked total/change bar shows; guard banner doesn't break the footer | [ yes] |
...
| 6.3 | Hard gate — Simple / Split / Collect | Same over-remaining entry on each surface | Cap + amber hint (Collect); Split via engine cap; Simple shows amountCapHint | [ yes] |
| 6.4 | Cash change exempt | Toggle OFF; cash tender > due | Change returned; no forced toggle; applied capped | [ yes] | the validate payment button remain large and visible??? restyle it

| 6.5 | Toggle ON without allocate perm | User without `orders:overpayment_allocate` clicks toggle | Toast/message with permission **name + code**; toggle stays OFF | [ yes] |
| 6.6 | Toggle OFF while extra | Enable toggle, create extra, try turn OFF | Blocked; message names reduce-legs or validate/route; amounts unchanged | [ yes] |
| 6.7 | Validate + route | Toggle ON + Validate + choose destination | Emerald Extra mirror; submit succeeds | [ yes] | 
| 6.8 | Legacy stuck-excess card | Force excess with toggle OFF (if reachable) | ExtraReceiptHandlingCard still usable; no silent leg rewrite | [ N/A] | 
| 6.9 | Tablet | Narrow window | Strip + banners + keypad usable; receipt slide-over OK | [ yes] | 
| 6.10 | Server guard | Trigger overpayment server rejection | Guard still opens routing dialog; strip consistent | [ yes] | 
...
| # | Check | Steps | Expected | Notes / result |
|---|-------|-------|----------|----------------|
| 7.1 | Advanced→Simple retarget | Advanced: add Cash + Card + Stripe (fill Stripe details); leave Stripe active; switch **Simple** | Simple amount/details show a **chip-visible** leg (Cash first if present), **not** Stripe gateway fields; Cash chip teal/`aria-pressed`; Card may show lighter "has leg" style | [yes ] |

| 7.2 | Stripe state preserved | After 7.1, switch back to Advanced; select Stripe leg | Amount + gateway txn/ref still present (engine state survived; no silent money rewrite) | [ yes] |
| 7.3 | Off-chip-only hint | Advanced: only Check + Stripe legs; switch Simple | Amount editor empty/disabled; info hint about Advanced method; More options still works | [ yes] |
| 7.4 | Advanced method highlight | Advanced with Cash + Stripe; activate Stripe | Only Stripe method card is primary-selected; Cash shows secondary "has leg" (not both primary) | [ yes] |
| 7.5 | Split gateway identity | Split dialog: two gateway options (e.g. STRIPE + HYPERPAY); Stripe leg with details; change method to Cash | Option lookup keeps correct gateway row; method change sets method + clears `gateway_code`; **amount unchanged** | [ yes] |
| 7.6 | Cap hint gated | Off-chip active in Advanced with a cap notice → Simple with no chip-visible active | Cap hint hidden on Simple until a chip method is active | [ yes] |

...
- **Remaining dialogs (build one per commit, each extends i18n + tests):** overpayment (wrap existing `payment-extra-receipt-dialog`), FX/rounding inline line, PAY_LATER (outstanding-policy selector). Then registry `Dialog` wiring.

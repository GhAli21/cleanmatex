...
3. **Reverse (cash):** same as above, but the dialog additionally requires picking an OPEN cash-drawer session; the service re-verifies that session is still OPEN server-side and creates a `PAYMENT_REVERSAL` OUT movement (linked via `reversed_payment_id`) as the auditable/recon compensating fact — atomic in the same transaction as the status flip. Expected cash drops **once** because the payment left the COMPLETED ledger term; the movement is excluded from the B35 MANUAL term so it does not double-decrement. Missing/closed session throws a stable error code the dialog surfaces as a blocking message (no silent mutation).
...
* **REVERSE** — legal from `COMPLETED`/`CAPTURED`/`SETTLED` → `REVERSED`. Mandatory reason, no fallback. Cash-family legs require an operator-supplied **OPEN** `cashDrawerSessionId` (re-verified server-side, same "no silent money mutation" pattern as B9's cash-refund execution) and get a real compensating `PAYMENT_REVERSAL` OUT movement in `org_cash_drawer_movements_dtl`, linked via `reversed_payment_id` (deliberately **not** `order_payment_id` — CASH_SALE already owns that unique payment link; recon looks up `reversed_payment_id`). The payment leaving COMPLETED is what drops expected cash; the OUT is recon/audit only and is excluded from the B35 MANUAL term (QA §30.2 over-decrement 1.070 → −1.070 if counted). Non-cash legs (card/bank/gateway/check) flip status only — gateway-side reversal is B8, out of scope.

...
| Next action | **B34 CLOSED (IMPLEMENTED 2026-07-18):** consolidated gates all green (eslint 0, jest 1946/1960 — the 14 fails are the 4 known pre-existing owner suites, build ✓, check:i18n ✓, tsc clean except the 2 known pre-existing owner-file errors); UI-access-contract wire checks PASS for `/dashboard/internal_fin/refunds` and `/dashboard/orders/[id]`; sync drift 0; platform inventories validate 10/10 (run contract checks from repo root with PowerShell — Git Bash mangles the route arg). **Baseline committed + tagged per owner directive 2026-07-18** (see the master README Next action + tag `order-fin-approved-baseline-2026-07-18`). **Dependency order recalculated 2026-07-18:** Seq 1–2 done; no decision gates anywhere; one scoped package per phase (owner directive 2026-07-18). **B15 IMPLEMENTED 2026-07-18** (same day, after the baseline tag — B15 changes are uncommitted in the working tree; see B15 Completion evidence; gates ALL GREEN: tsc clean/eslint 0/i18n ✓/targeted 38-38 + guard suite/full jest 1967-1981 [14 fails = the 4 known pre-existing owner suites]/build ✓). **NOW = next unblocked: B16 (Drawer filtering + variance approval), then B20 → B29.** Per package: read the Bxx file fully, load domain skills before code, evidence + statuses after every task; migrations author → **STOP-AND-WAIT** (mark BLOCKED-ON-APPLY). Owner queue: apply any queued migrations, Preview deploy → QA (B01 §14; B02 equality; B33 D9-PENDING stays CURRENT; B34 flag-on QA) → approvals → VERIFIED. Known pre-existing failures outside the program (owner keypad/split-payment/promo commits): 14 jest (cash-drawer ×2, order-calculation, credit-note-picker-focus) + 2 tsc (cmx-keypad test, split-tender-dialog) |

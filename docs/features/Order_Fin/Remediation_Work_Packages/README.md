...
| B16 | Drawer filtering + variance approval | MEDIUM | CONTROL_GAP | IMPLEMENTED 2026-07-18 — Part A (expected-cash filter) + Part B (deferred variance approval, mig 0407 applied by owner) both done, flag-gated `order_fin_drawer_close_v2`, gates green (tsc/eslint/i18n/jest 39-39/build ✓); Addendum A2 (cash-sale double-count, separate issue) recorded — awaiting owner commit → Preview QA | D001 | — | — | 3 | [B16](B16_Cash_Drawer_Filtering_And_Variance_Approval.md) | pending | — | pending Preview QA |
...
Seq 3  ← IN PROGRESS · Low-risk parallel wave: B15 ✅ IMPLEMENTED · B16 ✅ IMPLEMENTED
       (Part A filter + Part B deferred variance approval, both flag-gated, mig 0407
       applied) · B20 ← NOW · B29 next

...
## Metadata
Backlog ID: B13 · Severity: HIGH · Classification: CONTROL_GAP · Status: **IMPLEMENTED 2026-09-17 (v1 ORDER_PAYMENT unwind)** — flag `order_fin_voucher_unwind` default OFF; migration **0506 STOP-AND-WAIT**. Remaining: credit/SV unwind + preview UI. Not VERIFIED.
...
Rollout: apply migration 0506 → Preview enable per tenant → cash receipt reverse QA → then HQ default/override
...
**Migration `0506_add_feature_flag_order_fin_voucher_unwind.sql` — STOP-AND-WAIT.** Independent boolean, default FALSE. HQ docs + rollback script in cleanmatexsaas `docs/Added_Feature_Flags_docs/`. FLAG_CATALOG synced in the same change.
...
**Commit:** pending owner · **Preview QA:** blocked on applying 0506 then enabling the flag · **Verification:** not VERIFIED until cash receipt reverse + recon green on Preview.

...
3. **The ad-hoc `additionalTaxRate`/`additionalTaxAmount` params stay additive in both modes.** These only apply in the legacy no-tax-profile-configured fallback branch and represent a manually-entered order-level surcharge with no profile/catalog backing — never embedded in the priced item, so never extracted. `additionalTaxEmbedded = isInclusive && taxBreakdown.length > 0` is the single flag that discriminates the two cases.

...
## Metadata
Backlog ID: B16 · Severity: MEDIUM · Classification: CONTROL_GAP · Status: **IMPLEMENTED 2026-07-18** — **Part A + Part B**. **P1 settings field closed 2026-09-17:** `variance_approval_threshold` is on cash-drawer create/update (Payment Setup). Default close path remains owner-delegated VERIFIED 2026-09-16. §6.4–6.7 await Preview retest.
...
Frontend page/screen/dialog/action: drawer close screen — expected-cash breakdown shows only effective facts; variance-over-threshold triggers an optional approval dialog (permission holder + mandatory reason; closer may be the same person); session detail displays approval record
Reusable components/helpers: approval dialog pattern (shared with B24/B12)
Permissions: variance-approval code via B27
Validation: threshold from tenant config; permission is the only approval gate (same user may approve — no maker≠checker)
...
2. Variance within threshold → close completes; beyond threshold → optional approval dialog; a holder of `cash_drawer:approve_variance` confirms with reason (the closer may be the same person) → close records the closer and the approver (they may be the same user).
...
**Tests:** NEW `__tests__/services/cash-drawer-cash-facts.test.ts` (pure predicate — PENDING cheque / gateway PENDING·AUTHORIZED·PROCESSING / FAILED·CANCELLED·REVERSED / inactive / non-cash all excluded; COMPLETED·CAPTURED·SETTLED cash included) + `cash-drawer.service.test.ts` extended: Part A (flag OFF = unfiltered `where`; flag ON = active+COMPLETED-set+cash-family `where`) + Part B (`closeSession` pending-flag: OFF-even-over-threshold / ON-over-threshold-pending / ON-within-threshold-not-pending; `approveSessionVariance`: happy path, self-approval allowed, blank reason rejected, not-pending rejected, already-approved rejected, `VarianceApprovalError` shape). **Fixed the pre-existing owner failure** in `cash-drawer.service.test.ts` (added the missing `org_cash_drawer_movements_dtl.findMany` mock + the feature-flags `canAccess` mock). Targeted suites **39/39 green** (9 cash-facts + 30 service, updated from 30 after Part B additions).
...
**P1 settings field (2026-09-17):** `variance_approval_threshold` is now on cash-drawer create/update (`cash-drawer-schema.ts`, `cash-drawer-form-dialog.tsx`, `cash-drawers-actions.ts`, list column). Empty = no gate (same as NULL). EN/AR hint states optional / audit only. §6.4–6.7 become runnable from **Config → Payment Setup** without owner SQL.

**Commit:** owner reports prior work committed+deployed 2026-09-17; this UI follow-up pending next commit · **Preview QA:** default close path owner-delegated VERIFIED 2026-09-16; threshold path awaits retest after this deploy.

...
## Metadata
Backlog ID: B16 · Severity: MEDIUM · Classification: CONTROL_GAP · Status: **IMPLEMENTED 2026-07-18** — **Part A (expected-cash filter)** and **Part B (deferred variance approval)** both implemented, gates green (see Completion evidence) — awaiting owner commit → Preview QA. **Variance approval is OPTIONAL and non-blocking (owner directive 2026-07-18):** the close always completes; approval is opt-in per drawer (`variance_approval_threshold` NULL = no gate, the default) and, when a threshold is exceeded, a supervisor *may* record an approval for the audit trail — it is never mandatory and never blocks the close. Permission `cash_drawer:approve_variance` seeded by B27. **UPDATE 2026-07-18 (owner directive → [B35](B35_Unified_Drawer_Expected_Cash.md)):** the `order_fin_drawer_close_v2` feature flag was **removed** — the expected-cash filter (and the A2 double-count fix) now apply **unconditionally**; variance approval is gated only by the per-drawer `variance_approval_threshold` (opt-in, off by default). All B16 evidence below that references the flag reflects the original build; B35 supersedes the flag-gating.
...
**Commit:** — (owner) · **Preview QA (deploy/result/approval):** — pending (staging parity: filtered-vs-physical with `order_fin_drawer_close_v2` on; deferred-approval scenario — over-threshold close → pending banner → approval → approved banner. **No maker-checker as of 2026-08-14**: the closer may approve their own variance; `cash_drawer:approve_variance` (B27) is the only gate and must be seeded/granted for the action to be reachable) · **Reviewer:** — · **Verification:** — · **Authoritative report update:** Addendum A2 added 2026-07-18 (drawer cash-sale double-count — separate from this package, owner in-flight fix).

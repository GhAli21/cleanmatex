...
- `__tests__/validations/preview-submit-param-parity.test.ts` (new, 4/4 passing) — originally proved the preview-vs-submit request parity gap; **rewritten after the gap was fixed by removal** into a permanent regression guard that fails if a client-supplied tax override is ever re-introduced on submit without being wired into preview, plus assertions that `taxProfileIds` is accepted identically by both schemas and that the two remaining deliberate asymmetries (`promoCodeId`, `serviceCategories`) stay documented.
- `__tests__/db-integration/idempotency-claim-concurrency.db.test.ts` (new, 4/4 passing) — real 8-way concurrent proof of `claimIdempotencyKey`'s atomic claim (follow-up #5).
...
Commit: pending (owner) · **Preview QA: REQUIRED for the 2026-08-13/14 pass — see [QA_TEST_GUIDE.md](QA_TEST_GUIDE.md) §27.** (This package is normally test-only, but closing follow-ups #4 and #5 shipped two real production-behaviour changes — the ad-hoc tax-override removal and the `IDEMPOTENCY_IN_PROGRESS` rejection — so the earlier "Preview QA: n/a" no longer holds. Both are live for every tenant with no config gate.) · Reviewer: — · Verification: the *umbrella* is never "VERIFIED" in the usual sense (continuous by design), but the two behaviour changes in this pass follow the normal commit → Preview → QA → approval path · Authoritative report update: not filed separately, per this session's established precedent.

**Delivery-surface correction:** this file's `Delivery surfaces` block above declares `Backend services: NOT_APPLICABLE (test-only package)` and `Implementation files: none`. That was accurate until 2026-08-13. The 2026-08-14 state is: `lib/services/order-calculation.service.ts`, `lib/services/order-submit-orchestrator.service.ts`, `lib/validations/new-order-payment-schemas.ts`, `src/features/orders/hooks/use-order-submission.ts`, `lib/utils/idempotency.ts`, `lib/services/order-amendment.service.ts`, and one line of `lib/services/order-service.ts`. Recorded here rather than rewriting the block, because the umbrella's *standing* scope really is test-only — this pass was the documented exception, not a change of charter.

**Open items for the owner (neither urgent, both recorded rather than silently actioned):** (a) adopt `state_version` CAS in `updateOrder` once the workflow-engine WIP lands — the real residual behind follow-up #5; (b) let `submit-order/route.ts` and `workflow-engine.service.ts` adopt `claimIdempotencyKey` in their own reviewed pass (both were deliberately left untouched here).

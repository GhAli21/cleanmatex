...
**Commit:** — (owner commits) · **Preview QA (deploy/result/approval):** Preview pass 2026-09-12→2026-09-16 recorded in [QA_TEST_GUIDE.md](QA_TEST_GUIDE.md). §2.1/2.2/2.6 PASS. §2.3 BLOCKED then **code-fixed 2026-09-16** (B34 picker offers REFUND_AND_REBILL when `orders:rebill_authorize` is held). §2.5 **closed 2026-09-16 as guide correction** (ADR cancel allowlist — Cancel correctly hidden at `processing`; owner-authorized triage). Not VERIFIED. · **Reviewer:** — · **Verification:** — (VERIFIED requires Preview QA approval per §16) · **Authoritative report update:** —

...
| **Main** username / password | user=admin@demo-laundry.example And Password=Admin123 |
| **Limited** username / password *(fewer permissions)* | user=operator@demo-laundry.example And Password=Operator123 |
| **View-only** username / password *(optional)* | user=viewer@demo-laundry.example And Password=Viewer123 |
...
**Living document — updated after every implemented package.** Last update: **2026-09-17** (cancel = no auto Fin; B13 v2; 0443/0506/0507/**0508 + 0509 APPLIED**; reverse `RV-2026-000087` header backfilled; §30.2 expected-cash code-fixed). Previously 2026-08-15: §28 maker-checker removal.
...
| Action | Why |
|---|---|
| Confirm migrations **0443**, **0506**, **0507**, **0508**, **0509** are applied (owner, 2026-09-17, local + remote) | 0443 refund execution. 0506 voucher unwind flag. 0507 loyalty restore pending. 0508 `ref_voucher_id`. 0509 reverse-header backfill via `reversed_line_id` |
...
Pure SQL/DB verification, better run as a script than by a person: **§8.1/8.3** (injected recon drift), **§9** (B29 — docs-only), **§11** processor drain until `FINANCE_OUTBOX_SECRET` is set (UI rows 11.1–11.7 and hub 11.16–11.22 stay in the guide), **§20** job internals (UI smoke 20.1–20.4 already PASS; hub 20.12–20.18 after 0505). **§8.2 is now UI** (list + detail `total_checked`) — run it in Tier 2 after this deploy.
...
| Feature flags (HQ console) | `order_fin_refund_ui` = **ON**. `order_fin_sv_funding_capture` = **ON**. `order_fin_refund_execution` = **ON** (0443 catalog default true). `order_fin_voucher_unwind` = **ON on Preview** (owner enabled 2026-09-17); catalog default / other envs **OFF**. *(The old `order_fin_drawer_close_v2` flag was removed — B16/B35 drawer math is always on.)* **B30/B32 ship unconditionally, no feature flag.** |
...
| 11 — Background processing | 70–75 | §11.1–11.22 |
...
|2.5| **Cancel** a still-cancellable order (`draft` / `intake` / incomplete `preparing`) that has collected money | Status becomes cancelled. **Payments and credits do not move.** Dialog warns that money stays until Fin reverse/refund. No disposition chooser. Then handle money from Business Vouchers Reverse or Refunds. | BLOCKED historically at Processing (correct — Cancel is not allowed there). Retest on a draft/intake paid order after this deploy |
...
|6.4| **Config And Settings → Payment Setup** (`/dashboard/settings/payments`) → edit a cash drawer → set **Variance approval threshold** to `1.000` (hint says optional / audit only; empty = no gate) → **Internal Finance → Cash Drawers** → **Close** a session with a variance **over** 1.000 | Close still **completes** (never blocked); the closed session shows an **OPTIONAL** "supervisor approval available" banner (wording says *optional / for audit*, not "required") | **PASS 2026-09-17** — Payment Setup variance threshold set to `1.000`; over-threshold close still completed; optional supervisor approval path available. Unwind flag left OFF. |
|6.5| On that session, as the **closer**, click **Approve Variance** | **Approves successfully.** Maker-checker was removed (owner rule): holding `cash_drawer:approve_variance` is the only gate, and the closer may approve their own variance. A reason is still mandatory, and approval is still single-shot | **PASS 2026-09-17** — Same closer approved variance with reason successfully (maker-checker not required). |
|6.6| As a supervisor with the permission → **Approve Variance** → enter a reason | "Approved" banner shows approver + date + reason (a different user works too — it is simply no longer *required*) | **N/A 2026-09-17** — Needs a second supervisor login; not available in this run. |
|6.7| Close another session **within** the threshold | No approval banner appears | **PASS 2026-09-17** — Within-threshold close completed with no approval banner. |
...
> **B07 not signed 2026-09-17:** Outbox UI works but processor appears stalled (186 Pending, 0 Processed/24h; order events remain PENDING 0/6). Likely `FINANCE_OUTBOX_SECRET` / cron still missing on Preview — re-run §11.1–11.2 after fix. Ops-hub UI (filters, payload, bulk retry, jobs card): **§11.8–11.22**. Runbook: [FINANCE_JOBS_HUB.md](../Order_Fin_Docs/FINANCE_JOBS_HUB.md).
**What changed:** the financial outbox (loyalty points, order-history rows) now actually gets processed once a minute instead of sitting PENDING forever. Migration 0410 is **APPLIED (owner), verified via remote DB** — the remaining prerequisite is `FINANCE_OUTBOX_SECRET` still needing to be set from `sys_fin_runtime_cf` (see §0 Prerequisites) before drain (§11.1–11.2) is testable. The Outbox Monitor later grew into an ops hub (KPI tiles, filters, payload dialog, related links, bulk retry, auto-refresh) plus a Scheduled Jobs card (0505 APPLIED).
...
|11.19| Gift-Card Expiry / Credit-Note Expiry / ERP Posting Retry related links | Gift cards, Stored value, ERP exceptions respectively | **PASS 2026-09-17** — Related links present (Gift cards / Stored value / ERP exceptions). |
|11.20| **History** on any job | Dialog lists recent runs (started, trigger, status, outcome, duration, error). Processor history hides idle empty ticks except the latest heartbeat | **PASS 2026-09-17** — History dialog lists runs; idle empty SUCCESS ticks filtered; MANUAL/FAILED rows visible (15:38, 15:39). |
|11.21| **Run Now** (requires `finance_jobs:run`) | Confirm dialog first; on confirm the row pulses Running then SUCCESS/FAILED; overlapping Run Now while RUNNING shows a warning and does not start a second run | **PARTIAL 2026-09-17** — Credit-Note Expiry Run Now SUCCESS (`0 processed, 0 failed`). Outbox Processor Run Now **FAILED** (Prisma `$queryRaw()` error). |
|11.22| Without `finance_jobs:run` / `finance_jobs:view` | Run Now hidden; whole jobs hub hidden without view | **N/A 2026-09-17** — Needs login without `finance_jobs:run` / `finance_jobs:view`. |
...
|13.3| **Reports & Analytics** + **Reconciliation** run without new errors | FAIL — RECON-2026-002 Failed with 24 tax blockers + 1 outbox warning (demo tax noise; see §8.1). Not a missing B20 check. |
...
> **B19 signed 2026-09-17** — Scheduled Jobs Run Now (original three) SUCCESS on Preview. Hub follow-up (0505 APPLIED, five jobs): **§20.12–20.20**. Loyalty points FIFO ledger + expiry job + real Loyalty tab (0511 APPLIED, six jobs): **§20.21–20.25**. Runbook: [FINANCE_JOBS_HUB.md](../Order_Fin_Docs/FINANCE_JOBS_HUB.md).

**What changed:** gift-card expiry now writes a real ledger row + attempts an ERP-Lite GL post (previously a competing raw cron silently flipped status only — retired in this package's migration). Two brand-new jobs also start running: idempotency-key cleanup (nothing existed before) and ERP posting-retry (logic existed, nothing ever called it). A **Scheduled Jobs** hub on the outbox ops screen later grew to include the outbox processor and ledger-aware credit-note expiry (migration **0505 APPLIED** owner 2026-09-17). The Exception Workbench gained a **Retry** button. The Pending Payments worklist's Age column now shows elapsed days instead of a raw timestamp. **2026-09-17 (owner-authorized full re-scope):** loyalty points expiry is now implemented too, behind a real FIFO lot-consumption ledger (migration **0511 APPLIED** owner 2026-09-17) — see §20.21–20.25. **Wallet points expiry remains NOT implemented** — no policy surface exists anywhere for it, a documented gap not a silent omission (see B19's own Completion evidence).
...
|20.12| **Scheduled Jobs** after 0505 | **Five** rows: Outbox Processor (every minute), Gift-Card Expiry (02:00), Credit-Note Expiry (02:05), Idempotency-Key Cleanup (03:00), ERP Posting Retry (hourly :15). Cron badges **Active**. (20.1 still records the pre-hub three-row smoke.) | **PASS 2026-09-17** — Five Scheduled Jobs rows present after 0505. |
|20.13| After 0505: Outbox Processor last-run timestamp | Updates about every minute once `FINANCE_OUTBOX_SECRET` + pg_cron `fin-outbox-processor` are healthy | **FAIL 2026-09-17** — Outbox Processor last-run advanced (to 15:39) but jobs-hub health banner reports failure — not healthy (`FINANCE_OUTBOX_SECRET` / cron / processor). |
|20.14| **History** on Outbox Processor | Idle empty SUCCESS ticks are not listed (except the latest heartbeat); MANUAL / FAILED / non-zero counts appear | **PASS 2026-09-17** — Outbox Processor History filters idle empty SUCCESS ticks; MANUAL/FAILED listed. |
|20.15| **Run Now** on Outbox Processor while a tick is RUNNING | Confirm, then warning that the job is already running (409) — no second RUNNING row | **N/A 2026-09-17** — Brief `Running...` seen; 409 already-running conflict not reproduced before completion. |
|20.16| **Run Now** on Credit-Note Expiry with no eligible notes | SUCCESS `0 processed, 0 failed` — a clean no-op, not an error | **PASS 2026-09-17** — Credit-Note Expiry Run Now SUCCESS `0 processed, 0 failed` (clean no-op). |
|20.17| Create an ACTIVE credit note with `expires_at` before today → **Run Now** Credit-Note Expiry | SUCCESS; note EXPIRED; `org_credit_note_txn_dtl` `EXPIRY` row; remaining_balance 0; **no invented GL**. Raw `expire-credit-notes` cron is gone | **N/A 2026-09-17** — No expired ACTIVE credit note seeded this run. |
|20.18| Credit-Note Expiry related link | Opens Stored value (`/dashboard/customers/stored-value`) | **PASS 2026-09-17** — Credit-Note Expiry related link opens Stored value. |
|20.19| Cron inactive / secret missing | Jobs hub warning banner lists the failing checks | |
|20.20| `finance_jobs:view` only (no `finance_jobs:run`) | History visible; Run Now hidden | |
|20.21| **(NEW 2026-09-17)** **Scheduled Jobs** after 0511 | **Six** rows — the five from 20.12 plus **Loyalty Points Expiry** (02:10 daily). Related link opens **Marketing → Loyalty** | |
|20.22| **(NEW)** With no loyalty program configured (or `points_expiry_days` empty) for the tenant → **Run Now** on Loyalty Points Expiry | SUCCESS `0 processed, 0 failed` — a clean no-op, dormant exactly like the other expiry jobs until an owner sets a policy | |
|20.23| **(NEW)** **Marketing → Loyalty** → set **Points expiry days** to a small number (e.g. `1`) and save. Earn loyalty points for a test customer (complete an order with a loyalty-earning line), then directly age the earn row's `created_at` past the window (owner/dev DB step — no UI for backdating) → **Run Now** Loyalty Points Expiry | SUCCESS with a non-zero processed count; the customer's loyalty balance decreases by exactly the expired amount; `org_loyalty_txn_dtl` gains one `EXPIRE` row; `org_loyalty_txn_allocs_dtl` gains one allocation row per lot that expired | |
|20.24| **(NEW)** Redeem loyalty points on an order for a customer with multiple separate earn events (e.g. from two different past orders) | Redemption succeeds normally (unchanged customer-facing behavior); on the DB, `org_loyalty_txn_allocs_dtl` shows the redemption drew from the **oldest** earn lot(s) first | |
|20.25| **(NEW)** **Customers → \[any customer with loyalty activity\] → Loyalty tab** (also **B2B Customers → \[customer\] → Loyalty**) | Real stat cards (Points Balance / Lifetime Earned / Current Tier — no longer hardcoded), a transaction history table (type badge, signed points, running balance, reference — no longer a static "No transactions yet"), and — if `points_expiry_days` is configured and the customer has points expiring within 30 days — a warning banner naming the amount and date | |
...
**Jobs hub (2026-09-17):** migration **0505 APPLIED (owner) local + remote**. Runbook: [FINANCE_JOBS_HUB.md](../Order_Fin_Docs/FINANCE_JOBS_HUB.md). Manual QA: §11.8–11.22 (ops hub) and §20.12–20.20 (five jobs / credit-note expiry / overlap).

**Loyalty points FIFO ledger + expiry job (2026-09-17, owner-authorized full re-scope):** migration **0511 APPLIED (owner) local + remote, verified** (`remaining_points` column, `org_loyalty_txn_allocs_dtl` table, `fin-loyalty-points-expiry` cron, extended `chk_fjrl_job_code` all confirmed present). Gates: tsc clean (0 errors) · eslint 0 · `check:i18n` ✓ · `check:access-contracts` 10/10 · full jest **314/314 suites, 2779/2779 tests, zero regressions** (`loyalty.service.test.ts` 38 tests incl. FIFO consumption/expiry; `finance-jobs.service.test.ts` +2) · `npm run build` ✓ (exit 0, full route manifest — one transient Windows Prisma file-lock EPERM on the first attempt, self-resolved on retry, not code-related). Manual QA: §20.21–20.25 above.
...
| # | Where + how | Expected | Result |
|---|---|---|---|
|22.1| With the pilot tenant/branch set to `TAX_INCLUSIVE` + flag ON: build a new order with items, open the payment modal | Each tax row shows the rate and amount with a **"— tax included"** suffix; the grand total equals the sum of the priced item amounts exactly — nothing added on top | **PASS 2026-09-17 retest** — TAX_INCLUSIVE live. Bathrobe/Robe `1.800` OMR; Municipal Fee 2% **— tax included** `0.034`; VAT 5% **— tax included** `0.084`; total tax `0.118`; grand total **`1.800`** (nothing added on top). Prior exclusive FAIL superseded. |
|22.2| Submit that order | Order submits without an `AMOUNT_MISMATCH` error (server recomputes identically to the preview); open the order's Financial tab → tax lines match the preview to the smallest currency unit | **PASS 2026-09-17 retest** — `ORD-20260917-0006` submitted (no `AMOUNT_MISMATCH`); Financial tab tax lines match preview to `0.001` (`0.034` / `0.084`). |
...
|26.11| **(NEW 2026-09-17)** With only `tax_registration_no` set for the pilot tenant (26.3 — trigger-config row NOT required for this path), open an eligible order with tax lines: **Orders → All Orders → click order → Financial tab → Tax Document card**. With `tax_document:issue` permission, an **"Issue tax document"** button appears when the order has no document yet. Click it, pick **Simplified invoice** or **Invoice**, confirm | Document is created + issued (same as the automatic trigger); card now shows status ISSUED, document no., and a **"View / Print"** link; `cmxMessage` success toast | |
|26.12| Repeat 26.11 on an order that already has a document (from 26.4 or 26.11) | Button is hidden (order already has a document); if invoked directly against the API, `TAX_DOCUMENT_ALREADY_EXISTS` | |
|26.13| Click **"View / Print"** on an ISSUED document (from 26.4, 26.7, 26.8, or 26.11) | Opens `/dashboard/orders/{id}/tax-documents/{documentId}/print` — bilingual (EN/AR toggle), shows seller (tenant/branch name + tax registration no.), customer, order ref, document no./status/type, tax-line table (or "no lines" if the automatic-trigger path was used, which doesn't populate lines — a known pre-existing simplification), totals, and a verification QR code (generic — not a jurisdiction-specific compliance QR, see B14 Design decision #10). **Print** button triggers the browser print dialog with clean print-media styling (no nav chrome) | |
...
**Frontend follow-up gates (2026-09-17):** tsc clean (0 errors) · eslint 0 (targeted, all new/changed files) · `check:i18n` ✓ (no new warnings) · `check:access-contracts` 10/10 (new print route registered, nav↔contract drift 0) · full jest 314/314 suites, 2762/2762 tests (same session as B18's charge-void addendum, zero regressions) · `npm run build` ✓ (exit 0, full route manifest). New scenarios: 26.11–26.13 above.
...
|28.4| **Cash Drawers** — close a session with a variance over the drawer's threshold (set via Payment Setup — §6.4), then **as the same user who closed it**, click **Approve Variance** and enter a reason | Approves successfully (see §6.5) | **PASS 2026-09-17** — `DRW-BR2-001` threshold `1.000`; `SES-000017` closed physical `2.000` (variance `2.000` over threshold); same-user Approve Variance with reason succeeded; audit banner confirms approval. |
|28.5| On that same session, click **Approve Variance** again | Rejected as **already approved** — single-shot approval is still enforced | **PASS 2026-09-17** — `SES-000017` no longer exposes Approve Variance after approval (single-shot). |
|28.6| Attempt a variance approval with an **empty** reason | Rejected — a reason is still mandatory | **PASS 2026-09-17** — `SES-000019` over-threshold close; empty-reason Approve blocked (reason mandatory); approval stayed pending until reason provided. |
|28.7| Log in as a user **without** `cash_drawer:approve_variance` → open the pending-variance session | No approve action available / permission error | **PASS 2026-09-17** — Operator Demo 1 on `SES-000019`: Approve Variance disabled / no usable approve action. |
...
## 30. B13 — Voucher reversal operational unwind

**What changed (2026-09-17 v2):** reversing a posted voucher used to be paperwork only. Behind **`order_fin_voucher_unwind`** (0506 APPLIED, default **false**), Reverse also VOID/REVERSE linked **ORDER_PAYMENT** legs (B10), restores **ORDER_CREDIT_APPLICATION** (D006), and claws back unused stored-value funding (`WALLET_TOPUP` / `GIFT_CARD_SALE` / `CUSTOMER_ADVANCE_RECEIPT` / `CUSTOMER_CREDIT_ISSUE`). Reverse dialog shows a consequence preview. **0507 APPLIED** (`LOYALTY_RESTORE_PENDING` can persist).

> Where: **Internal Finance And Operations → Business Vouchers** (`/dashboard/internal_fin/vouchers`) → open a **POSTED RECEIPT** → **Reverse**. Permission: `fin_vouchers:reverse`. Cash reverse needs an **OPEN** session on the **original drawer**.

> Enable the flag **on Preview only**. Leave it OFF in production until this section PASSes. Cancel an order first if needed — cancel itself does **not** unwind money.

| # | Where + how | Expected | Result |
|---|---|---|---|
|30.1| Flag **OFF**. Open a posted cash RECEIPT from collect-payment → **Reverse** with a reason | Reverse succeeds. Preview says document-only. Original payment stays COMPLETED. Reversal lines `NOT_WIRED`. Outstanding **unchanged**. | **N/A 2026-09-17** — Flag left **ON** for Preview (owner); document-only OFF path not re-run. |
|30.2| HQ: enable `order_fin_voucher_unwind` for Preview. Reverse a **different** posted **CASH** RECEIPT while that drawer's session is **OPEN** | Preview lists the payment. Payment REVERSED/VOID. Reversal `ORDER_PAYMENT` line `WIRED`. Outstanding reopens. Drawer expected-cash **drops once** (opening + remaining cash facts). If that cash was the only session fact, expected is **0.000**, never the negative of the cash amount. | **PASS 2026-09-17 retest** — Cash fix verified. `ORD-20260912-0009` `1.070` OMR; voucher `RV-2026-000088` → reversal `RV-2026-000089`; payment REVERSED; WIRED; outstanding reopened; compensating PAYMENT_REVERSAL OUT on `SES-000016` (OPEN). Expected-cash **`1.070` → `0.000`** (not `-1.070`). Prior FAIL superseded. |
|30.3| Flag ON. Reverse a posted **CARD** RECEIPT | Payment REVERSED/VOIDED, line WIRED, outstanding reopens, **no** drawer movement | **PASS 2026-09-17** — Card receipt `RV-2026-000090` → reversal `RV-2026-000091`; payment REVERSED; outstanding reopened on `ORD-20260912-0009`; no cash-drawer movement for card path. |
|30.4| Flag ON. Reverse posted **CASH** with **no OPEN session** on the original drawer | Whole reverse **aborts**. Original stays POSTED. Drawer-session-required error (EN/AR). | **PASS 2026-09-17** — After closing `SES-000016`, cash reverse attempt aborted / required open session; original voucher remained **POSTED** (no B10 unwind). |
|30.5| Flag ON. Reverse a voucher with **ORDER_CREDIT_APPLICATION** (wallet/GC/advance/CN) | Credit app REVERSED; source ledger restored; reversal line WIRED; order outstanding reopens by the credit amount | **PASS 2026-09-17** — Advance/`ORDER_CREDIT_APPLICATION` on `ORD-20260917-0003` `1.926` OMR; `RV-2026-000094` → `RV-2026-000095`; credit app REVERSED; advance ledger restored (`55.127`); order balance due `1.926`, credits `0.000`. |
|30.6| Flag ON. Reverse a **WALLET_TOPUP** / gift-card sale / advance receipt whose balance is still unused | Funding clawed back (wallet/advance down, gift card VOIDED). Tender status REVERSED. | **PASS 2026-09-17** — Unused gift-card sale `RV-2026-000083` reversed → `RV-2026-000093`; funding claw-back path exercised (tender reversed / card void path). |
|30.7| Flag ON. Reverse a gift-card sale / wallet top-up **after** the customer spent it | Reverse **aborts** with already-used / insufficient error. Original voucher stays POSTED. | **PASS 2026-09-17** — Spent gift-card path: funding `RV-2026-000096` (5.000 OMR); spent 1.000 on `ORD-20260917-0004` (order 1.070, POC 0.070); reverse aborted with exact error: "This gift card has already been spent. Reverse cancelled."; voucher stayed **POSTED**; no claw-back/void; order payment remained. |
...
| B13 | migrations **0506 + 0507 + 0508 + 0509 APPLIED**; flag `order_fin_voucher_unwind` default **false**; v2 unwind + preview in code; reverse header/`ref_voucher_id` backfilled on `RV-2026-000087`; §30.2 expected-cash double-count **code-fixed 2026-09-17** | **Preview §30.2 PASS (cash expected-cash fix retest); 30.3–30.6/30.8–30.10 PASS; 30.1 N/A (flag ON); 30.7 PASS** | Financial_Expert_Tester / 2026-09-17 |
...
| B11 | no migration (schema/flag pre-existed from 0339); requires direct DB config (no settings UI) to opt a pilot tenant into TAX_INCLUSIVE before §22 is runnable on Preview | **PASS Preview 2026-09-17** — §22.1–22.2 after tenant Tax Pricing Mode = TAX_INCLUSIVE + flag override ON | Financial_Expert_Tester / 2026-09-17 |
...
| Maker-checker removal | no migration, no DB change. Removes the "a different user must approve" rule from refunds AND drawer-variance approval; permission (`orders:approve_refund` / `cash_drawer:approve_variance`) is now the only gate. Live for every tenant immediately, no config. **§5.4 and §6.5 now expect the OPPOSITE of previous revisions** — a tester reporting "I could approve my own request" is reporting correct behaviour. §28 covers it end to end | **PASS Preview 2026-09-17** — §28.1–28.3 prior PASS; §28.4–28.7 PASS (same-user approve, single-shot, empty reason, operator gated) | Financial_Expert_Tester / 2026-09-17 |

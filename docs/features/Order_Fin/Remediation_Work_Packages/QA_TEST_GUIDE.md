...
| # | Where + how | Expected | Result |
|---|---|---|---|
|26.1| (Owner/dev only) `SELECT tax_registration_no FROM org_tenants_mst WHERE id = '<pilot tenant>';` — confirm NULL by default | NULL for every tenant (migration adds the column with no backfill) | |

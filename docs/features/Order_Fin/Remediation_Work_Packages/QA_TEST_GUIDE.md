...
**What changed:** the new-order page's Preferences → Service tab gained a "Whole Order" preference section (order-wide extras, e.g. a rush/handling fee). Both the order-level and item-level sections were then **redesigned** (owner feedback: the first cut only offered one preference kind) into a "kind toolbar" — a row of buttons, one per configured preference kind, that opens a picker dialog — the same interaction pattern piece-level preferences already use elsewhere in the app. Item-level now additionally offers **packing preference** through the same toolbar (previously only editable elsewhere). Every preference with a non-zero amount — order, item, or piece level — writes a matching fact row into the order's charges ledger, closing a real gap where that ledger was always empty.
...
| # | Where + how | Expected | Result |
|---|---|---|---|
|24.1| New order → add at least one item → Preferences → Service tab | A **"Whole Order"** section appears above the per-item list, showing a row of kind buttons (e.g. "Service Preferences") rather than a flat list | |
|24.2| In the "Whole Order" section, click the "Service Preferences" kind button | A dialog opens listing available preferences with prices; selecting one closes the dialog and shows it as a removable chip on the card | |
|24.3| Add a "Whole Order" preference with a non-zero price | The order total updates live to include the amount; the item subtotal is unaffected (the charge is a separate line internally); the chip shows the price | |
|24.4| Remove a chip via its × button | The preference is removed and the total updates live | |
|24.5| For any item, look at its row inside the Service Preferences tab | The item shows its own kind toolbar (not just a flat selector) offering **both Service Preferences and Packing** — pick a packing option and confirm it appears as a chip with its price | |
|24.6| For a tenant that tracks by piece: open an item that has pieces | The item's own toolbar (service prefs + packing) appears labeled **"Whole item"**, in addition to the per-piece selectors below it — both can be used together, not one-or-the-other | |
|24.7| Submit an order with an order-level preference, an item-level packing choice, and at least one piece-level preference | Order submits without error; open the order's Financial tab — the total matches what was previewed | |
|24.8| (Owner/finance only) Run reconciliation for a NEWLY submitted order from this build that has a preference charge | `ORDER_CHARGES_MATCH_SNAPSHOT`, `ORDER_PIECES_MATCH_CHARGES`, `ORDER_PREFERENCES_MATCH_CHARGES`, and `PREFERENCE_EXTRA_PRICE_INCLUDED_ONCE` are all clean for that order (see the "known gap" note above for orders predating this build) | |

**Automated gates at build time (2026-07-25, B18 final — post-redesign):** tsc clean (3 pre-existing unrelated errors, same as every prior package this session) · eslint 0 (project-wide) · `order-calculation.service.test.ts` +4 new (no-op default, flat non-taxable addend, gift-card cap consistency, combined with B17 rounding) · `check-modules.test.ts` +1 new (proves the exact write-shape — one charge row per preference with `charge_source_id` lineage — passes all five `order-snapshot-checks` reconciliation checks cleanly) · full jest **237/237 suites, 2300/2300 tests — zero known failures** · `check:i18n` ✓ (no new keys — the redesign reuses the existing `newOrder.piecePreferences.*` translations piece-level already had) · **`npm run build` ✓ (exit 0)** — the earlier unrelated assembly-exceptions build blocker (owner's own WIP) is now resolved. §24 above is fully runnable once deployed to Preview.

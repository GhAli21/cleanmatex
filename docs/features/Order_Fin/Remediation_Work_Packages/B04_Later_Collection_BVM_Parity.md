...
**Two deferrals recorded here were closed later by the Collect Payment Enhancement (2026-08-15)** — see [`../Collect_Payment_Enhancement/STATUS.md`](../Collect_Payment_Enhancement/STATUS.md), QA §29:

1. *"no voucher-receipt print wired — deferred, no existing collect-receipt UI to extend without new scope"* (Delivery surfaces line) — **closed**. The modal gained an optional `onPrintReceipt?` prop; the Ready screen wires it to its existing `openPrintPreview('receipt','thermal')`. The other two mounts omit it and the control does not render, so no surface-sniffing was introduced.
2. *"Full validation parity … reference/terminal checks … NOT implemented — collect's request contract has no fields to satisfy those checks against"* (Validation line) — **partially closed for reference**. The route contract now carries `reference`, `checkNumber`, `checkBank`, `checkDate` via a shared `collectionPaymentLegSchema`, the UI renders them per method, and `requires_reference` is enforced client-side before submit; the check due-date rule reuses submit's own `validateCheckDueDate`. **Terminal/gateway-config parity remains open** — collect still has no `terminalId` field.

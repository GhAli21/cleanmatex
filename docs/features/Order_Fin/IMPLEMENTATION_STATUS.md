...
- **B18 addendum** — charge void action + a backfill migration (`0510_b18_charge_backfill.sql`) for pre-B18 orders missing their charge-ledger facts. **Migration 0510 APPLIED (owner, 2026-09-17) local + remote, verified: 0 rows backfilled** (no orphan preferences existed).
...
- **B19 addendum (owner reversed an earlier same-session skip decision)** — a loyalty points FIFO lot-consumption ledger (`org_loyalty_txn_dtl.remaining_points` + new `org_loyalty_txn_allocs_dtl` allocation table), a `loyalty_points_expiry` scheduled job on the existing finance-jobs-hub infrastructure, and a real customer-facing Loyalty tab (previously two hardcoded stub components wired to nothing). **Migration 0511 APPLIED (owner, 2026-09-17) local + remote, verified.**

Deliberately **not** built this session (owner-scoped out): B12's automated settlement-collection dialog (real architecture blocker — `collectPaymentTx` is PAY_ON_COLLECTION-scoped only). Wallet points expiry also remains out of scope — no policy surface exists anywhere for it.

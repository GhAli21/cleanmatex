...
1. **`unwindOrderFinancialsTx`** (settlement-owned, same-tx composable): reverse APPLIED credit apps → `REVERSED` (constant, not literal); restore stored value (wallet/advance/gift via existing ledger services, idempotency-keyed `cancel-<orderId>-…`); route real payments per chosen disposition — **Refund** (existing `order-refund.service` initiate/approve/process; permission-gated, same user may approve — no maker≠checker) or **Store credit** (issue credit note / wallet top-up); **Keep-on-account** writes an overpayment-disposition row and requires `orders:approve_refund`; then `recalculateOrderFinancialSnapshotTx` + audit row + outbox event.

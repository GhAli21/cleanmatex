...
**Program status:** Wave 0 (Foundation) **COMPLETE** — all migrations (0515–0518) applied local+remote. Wave A **IN PROGRESS** — A1 (session numbering) implemented, migration `0519` awaiting apply.
...
| A | Money & concurrency integrity | **IN PROGRESS** (A1 **COMPLETE**, A2 **COMPLETE** except the documented payment-during-close gap — D22, A3/A4/A6 not started) | 0519 (session numbering); A2 needed no migration | **APPLIED** (local + remote) 2026-09-23 |
...
| A3 | ⬜ Not started | Decimal-only cash computation (breaking-test sweep + demo data recompute) |
| A4 | ⬜ Not started | Multi-currency correctness (`org_cash_sess_curr_dtl`, migration pending) |

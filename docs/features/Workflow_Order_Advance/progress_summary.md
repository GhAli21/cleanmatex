...
**Updated:** 2026-09-12  
**Overall:** Create hydration leftover close-out **done**. Catalog **1.3.0**. `WF_V2_HOME_COLLECTION` v1 is **PILOT** on remote. **HC1 + HC2 CLOSED.** **S10 SIGNED (2026-09-05).** Delivery Feature Completion Phases 1–6 implemented; delivery-route / Ready collect-payment / hold H1–H4 smokes **done**. Quick-drop → `preparing` is live (`0499`+`0500` applied; HQ Check policy on SIMPLE v4 **passed**). Preparation header **Edit Order** shipped. Order details + workspace Actions now mount **New order actions** (`new_order`) and **Order actions** (`order_control`). Leftover-intake ActionBar smoke and quick-drop floor smoke **operator-confirmed done 2026-09-12**. SIMPLE leftover dest **accepted as-is** (later dest edits: HQ Studio). **`lwpr-tenant-docs-final` done 2026-09-12.** Remaining operator watch: HQ **soak**. V1.0.x+ listed in `current_status.md`.

**Delivery Feature Completion update:** `0490` and `0491` are applied locally and remotely. Phases 1–4 are implemented: tenant driver master data, race-safe atomic route commands, Drivers CRUD, and the dispatcher route-planning workspace with shared searchable driver reassignment. The owner approved Phase 5, so `STAFF_DELIVERY_WRITES_ENABLED=true` exposes only the hardened route command APIs; legacy POD/OTP APIs remain deleted. Phase 6 documentation, contract inventory, and automated verification are refreshed. **Delivery route UI smoke — operator confirmed done 2026-09-11.**

## Accurate status

The block below is a **2026-08 implementation diary**. Runtime law today is live profile-version rows (`WorkflowPolicyResolver`) and HQ **Check policy**. Compiled artifacts and graph pins are retired. Treat dated “artifact / compile / checksum” sentences as history, not current contract.
...
Absolute semantic-only cutover: engine, workflow context, Workboard, floor lists, client transition adapter, and new-order initial status no longer read graph pins, templates, live catalogs, status filters, or fallback statuses. Unsnapshotted historic/test orders fail closed operationally and must be recreated; migration `0464` is applied locally and remotely and prevents future incomplete active order snapshots. HQ effective preview/tenant simulation read live profile-version rows (`policy_revision`); legacy HQ authoring writes are retired with `LEGACY_WORKFLOW_RETIRED`.
Automated semantic-profile assurance: Pilot only on HQ test/demo tenants; latest assignment stays PUBLISHED; forged screen/channel rejected; PROFILE_* integrity maps to HTTP 409. Residual: performance soak. S10 signed. `lwpr-tenant-docs-final` done.
...
- [x] P7R Workboard: tenant-safe read model/API, RBAC/access contract, EN/AR Cmx screen, supervisor filters, and owner-stage routing from the order-bound live profile version (legacy compatibility retained temporarily)
- [x] P0 semantic profile snapshot: new orders persist profile/version identity (`wf_profile_id` / `wf_version_no` / `wf_profile_version_id`); Gate 5 retired compiled artifact id/checksum as runtime. Loader fail-closes incomplete bindings.
...
- [x] P0 semantic runtime enforcement: action list/execution, Workboard, and floor worklists use live profile-version rows for bound orders; graph-pin execution is retired; unbound historic orders fail closed operationally
...
- [x] P0 semantic context: V2 floor-page actions no longer guess destinations from template flags; workflow-context is a display-only projection of the order-bound live version; legacy template context is isolated to unbound historic orders
...
- [x] Make the explicit staff S10 rollout decision after e2e/canary (`p7-harden`) — S10 SIGNED 2026-09-05
- [x] Repeat production cancel/hold/resume/stop smoke after `0442` — **V1.0:** remote demo is the production-like environment (owner 2026-09-11). Hold H1–H4 floor smoke **done**. S10 **signed**. Do not keep this open for a future paying tenant.
- [x] Sign pilot T01-T18 and rollback rehearsal — T15 stays 🟡/owner-deferred; rollback rehearsal operator-confirmed 2026-09-11 (Workboard = per-order pin, not current assignment)

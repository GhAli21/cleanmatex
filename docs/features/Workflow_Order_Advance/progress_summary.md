...
**Updated:** 2026-09-11  
**Overall:** Create hydration leftover close-out **done**. Catalog **1.3.0**. `WF_V2_HOME_COLLECTION` v1 is **PILOT** on remote, assigned to tenant `c9ac29d1-219c-4a3a-8887-f860550c32be`. Found + fixed 2 real bugs (tenant-side `semanticEvidenceSchema` missing `fulfilment_channel='home_collection'` which hid every floor screen for this profile's orders; `WorkflowActionBar` missing `FAIL_HOME_COLLECTION` from its reason-prompt set) and confirmed one operator policy fix in Studio (`CONFIRM_HOME_COLLECTION.to_status` changed `intake`→`preparing`, resolving a stuck-at-intake gap for this flow). **HC1 + HC2 both CLOSED**, confirmed on remote via order-history audit trail. Regression-checked clean on `WF_V2_SIMPLE` v4. **S10 staff routed POD canary SIGNED (2026-09-05)** — real operator, real UI, `ORD-20260903-0005`, closing the longest-standing programme blocker. Generalized the reason/notes field across `WorkflowActionBar` (data-driven from `requires_reason`/`min_reason_length`, retiring the hardcoded action list). Fixed a stale DB-integration test (`delivery-completion.db.test.ts`, was silently 4/9-failing since Gate 4) — 9/9 passing again. **New: Delivery Feature Completion programme started** — S10 was only reachable via a manual SQL seed because route creation/driver assignment are hard-disabled on a legacy, buggy service; full plan approved (`C:\Users\JHNLP\.claude\plans\merry-singing-pike.md`), Phase 1 (schema: `org_drivers_mst`, route/stop safety constraints) drafted as migration `0490_org_drivers_mst_and_route_safety.sql` — **awaiting operator apply**. **Delivery route / collect-payment / hold smokes (2026-09-11):** delivery route UI **done**. Collect-payment UI on Ready **done**. Hold floor H1–H4 **done**. Quick-drop → `preparing` is live (`0499`+`0500` applied; HQ Check policy on SIMPLE v4 **passed** after HQ deploy). Preparation details header **Edit Order** shipped. No intake ActionBar. Remaining V1.0 operator work: quick-drop smoke.

...
- DB minimum check: `sys_wf_prof_ver_live_rpt` + `sys_wf_prof_ver_validate_live` in `0477` (after `0470`/`0476`). Structural only; it does **not** replace the catalog. Named observer-execute exceptions: pickup `CONFIRM_PICKUP` from observed `ready`, and public `CONFIRM_DELIVERY` from observed OFD. Do not edit applied 0470–0476.

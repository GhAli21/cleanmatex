...
- Preparation floor:
  ...
  - Migration `0501_wf_simple_intake_to_preparing.sql` (**applied as no-op**; leftover SIMPLE dest **accepted as-is** — later dest edits in HQ Studio)
...
- Simple vs routed delivery is live profile policy. Catalog already has `CONFIRM_DELIVERY` on `driver_delivery` with no transition `gate_set_code`. HQ binds `delivery_stop_active` / POD evidence only for routed tenants. Do not seed that gate onto `TR_OFD_DELIV`.
...
- Floor queues must send `workflow_screen`. Do not rebuild membership from a client status list; bound orders can use statuses the live contract does not mention, and a live-contract status must not pull a bound order onto a screen its profile version does not own.
...
1. HQ **soak** (no click script). S10 is **SIGNED**. Do not start Gate 5 again.
2. V1.0.x (not V1.0 blockers): 46 planned Check-policy codes (HQ, owner review first); open-order migrate.
3. Implement durable B2B finance only in the B2B bounded context through the existing payment-hold seam. HQ can bind piece/QA/fulfilment/evidence gates (`0463` applied).

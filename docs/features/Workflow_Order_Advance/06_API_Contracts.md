...
`POST /api/v1/orders/submit-order` accepts `orderTypeId` and `orderSourceCode`. They default to `POS` and `pos` for staff New Order, are validated against the DB-mirror code lists, and then enter `OrderService` source-active/tenant-allow and Initial-rule validation. The public customer booking DTO continues to use `fulfillmentType`, which maps to `orderTypeId`; it does not add a second persisted field.

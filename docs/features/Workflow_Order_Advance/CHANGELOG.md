...
- **T03 closed — real gap found while manually testing retail order creation, not just documented.** The operator reported "no credit option, all pay same time" for retail while walking T01–T04. Investigated whether that's actually enforced or just a UI convention: `use-order-submission.ts` blocks `PAY_ON_COLLECTION` for retail-only orders client-side, but `OrderService.createOrder`/`.createOrderInTransaction` never rejected it server-side, and retail's initial-status resolution lands straight at the terminal `delivered` status (RETAIL_SOLD preset) with no later collection gate — unlike laundry orders, which are still gated by `PICKUP_COLLECTION_REQUIRED`/`DELIVERY_COLLECTION_REQUIRED` before pickup/delivery. A direct or forged API call could have created a `delivered` retail order with an uncollected balance that nothing downstream would ever re-check — exactly the "silently auto-closed" scenario T03 exists to rule out. Fixed: both create paths now reject a retail-only order paid `PAY_ON_COLLECTION` with `errorCode: RETAIL_PAY_ON_COLLECTION_NOT_ALLOWED` before any write (order-number generation never reached). New `__tests__/services/order-service-retail-payment-guard.test.ts` (2/2 passing) proves the rejection for both paths. Gates: tsc clean, eslint clean on touched files. See `12_Test_Plan.md`/`13_Production_Readiness_Checklist.md` for the full T03 note.

## Unreleased — 2026-09-10 (4)

- **T07 closed; `PAY_ON_COLLECTION` re-verified as already implemented, not built from scratch.** Continuing the checklist backlog. T07's untested "partial release" half now has 2 real unit tests (`pickup-completion.service.test.ts`) covering both triggers of `PICKUP_PARTIAL_RELEASE_UNSUPPORTED`. Before writing any `PAY_ON_COLLECTION` code, ran a dedicated investigation rather than trusting the checklist's "not yet implemented"-sounding wording — the gate turned out to be real, wired into both `pickup-completion.service.ts` and `delivery-completion.service.ts`, and already unit/gate-tested. The one real remaining gap was proof of the *composed* block→collect→retry loop; added exactly that as a new DB-integration test in `delivery-completion.db.test.ts` against real local Postgres. See `13_Production_Readiness_Checklist.md`'s 2026-09-10 T07/PAY_ON_COLLECTION note for full detail. Gates: tsc clean, eslint clean, 9/9 + 14/14 relevant suites green.

## Unreleased — 2026-09-10 (3)

- **T05, T12, T13, T18 closed with real automated tests; T15 partially closed.** Working through the "what's remaining in the production readiness checklist" backlog one item at a time.
  - **T05 (Assembly scan gate)**: `__tests__/services/assembly-service.test.ts`, 10 tests against `AssemblyService.scanItem` (task-status gate, barcode-match/no-match, matching via either the assembly-item's own barcode or its linked order-item's barcode) and `completeAssemblyTask` (blocks on a pending item, blocks on an open exception, completes cleanly, idempotent replay, rejects a zero-item task). Follows the repo's established `mockFrom.mockImplementationOnce(...)` per-call chain convention (`order-service.test.ts`), with every call sequence traced from the real service code.
  - **T12 (Bulk/PATCH status denied or engine-only)**: `__tests__/api/v1/orders-bulk-status-retired.route.test.ts`, 4 tests — `POST /api/orders/bulk-status` always 410s with `USE_WORKFLOW_ACTIONS` for an authenticated tenant user without ever touching the database, 401 unauthenticated, 400 with no resolvable tenant; `[id]/route.ts` module asserted to export no `PATCH`/`PUT`/`DELETE`.
  - **T13 (Tenant cannot edit transitions)**: `__tests__/app/dashboard/settings/workflows-legacy-redirect.test.tsx`, 2 tests proving both legacy workflow-editor pages (`/new`, `[id]/edit`) redirect unconditionally to the read-only hub.
  - **T18 (EN/AR action labels) — real bug found and fixed**: `__tests__/services/workflow-engine-blocked-reasons.test.ts`, 20 tests. While writing coverage for the 3 existing bilingual blocked-reason builders, found a 4th one inline in `executeAction` (`WF_GATE_ACK_REQUIRED`, the warning-acknowledgement-required reason) with **no Arabic branch and no `message2` at all** — an Arabic-locale staff user blocked on a warning gate got an English-only reason with no translation, unlike every sibling blocked reason. Extracted it into an exported `warningAckBlockedReason(locale?)` matching the other three builders' established pattern; all 4 now share one 5-test group (EN default, EN explicit, AR swap, AR locale-variant tolerance, EN/AR never collide).
  - **T15 (Graph CI on seed) — partially closed**: `.github/workflows/ci.yml`'s `web-admin` job now runs `npm run test --workspace=web-admin -- --ci`. This closes a materially bigger gap than T15's own wording implied: the job previously only ran `lint` + `build` — **none of the 298 Jest suites / 2656 tests ran in CI at all**. The DB-integration graph validator itself (`wf-prof-ver-validate-live.db.test.ts`) is still not wired into CI — that needs a Postgres service container replaying all 504 migrations, a real infra/runtime-cost decision intentionally left open rather than added unilaterally.
  - **Two things fixed to land the CI step cleanly, both confirmed pre-existing/unrelated before touching them (`git status` showed no prior changes to either area this session)**: (1) `orders-transition.route.test.ts`'s "emits a notification for a fresh transition" test was asserting pre-hardcode behavior — `isOrderTransitionNotifyEnabled()` was hardcoded to `return false` in an earlier session (the owner's own deliberate change); updated the test to assert the current no-notify state with a comment marking it as the one line to flip back when the hardcode is lifted. (2) `/dashboard/orders/[id]/workspace` (from the in-progress order-workspace redesign) had no UI access contract; scaffolded it through the full `/rebuild-ui-access-contract` golden path (`scaffold` → `derive --apply` → `wire --fix` → `check --wire` → `sync`), gated on `orders:view_financial_breakdown` to match the sibling `/dashboard/orders/[id]` page. Found a real tooling bug while scaffolding: `orders-access.ts`'s `ORDERS_ACCESS_CONTRACTS` array closed with a bare `]` instead of `];`, silently breaking the scaffold script's closing-bracket search for *every* future route in that file, not just this one — fixed.
  - Gates: `web-admin` tsc clean, eslint clean, full `npm test` 298/298 suites (2656/2656 tests) green, `npm run build` green, `sync:ui-access-contract` 151/151 routes with 0 drift.

## Unreleased — 2026-09-10 (2)

- **T16 (RLS tenant isolation) closed with a real proof, not a mock.** Every prior "RLS" test in the repo mocked the Supabase client (`__tests__/tenant-isolation/rls-policies.test.ts`, `__tests__/auth/request-permission-auth.test.ts`) — none could ever fail even if the policies were broken or missing. Investigated why before writing anything: `DATABASE_URL` (Prisma, used by the entire workflow-engine/delivery/pickup service layer) connects as the Postgres `postgres` **superuser**, which unconditionally bypasses RLS — that's standard Postgres behavior, not a bug, and it means tenant isolation for that whole code path is enforced entirely by its own explicit `WHERE tenant_org_id = ...` clauses, never by RLS. RLS (`current_tenant_id()`, migration `0004`/`0081`, reading `auth.jwt() -> 'user_metadata' ->> 'tenant_org_id'`) is the real enforcement layer only for the separate set of API routes using `createServerSupabaseClient()`/`createClient()` (dozens under `app/api/v1/orders/**`), which run as the Postgres `authenticated` role under PostgREST.
  New `__tests__/db-integration/rls-tenant-isolation.db.test.ts`: each test opens a transaction, `SET LOCAL ROLE authenticated` (a superuser may switch to any role without a grant), sets `request.jwt.claims` via `set_config(...)` to the exact JSON shape PostgREST sets from a real JWT, then queries — genuinely bound by the same RLS policy (`tenant_isolation_org_orders_mst`, `FOR ALL USING (tenant_org_id = current_tenant_id())`) a live API request would be, then rolls back. 4/4 tests, run against real local Postgres (Docker/Supabase was down at the start of this work and had to be started to verify this rather than ship it unverified): same-tenant SELECT sees the row / cross-tenant SELECT does not (same query, proven both directions); cross-tenant UPDATE affects 0 rows with the target row proven unchanged; same-tenant UPDATE affects 1 row (so the test can't pass vacuously via "everything fails closed"). Gates: `tsc --noEmit` clean, `eslint --quiet` clean, full `test:db-integration` suite 94/95 (1 pre-existing unrelated failure — local seed-data staleness in `wf-policy-issue-catalog-seed-invariants.db.test.ts`, confirmed via `git status` as untouched by this change).
  `12_Test_Plan.md` T16 flipped ❌ → ✅ with a new "two isolation mechanisms" architecture note (the app-level vs. RLS split above, made explicit so it isn't rediscovered later). `13_Production_Readiness_Checklist.md` updated. The old mocked test file was left in place (superseded, not deleted — cheap, harmless, not worth the churn).

## Unreleased — 2026-09-05 (7)

- **`0494_wf_prof_ver_artifact_retirement.sql` applied — Gate 5 code-side work done.** Operator applied to local and remote. Verified directly on both (not taken on faith): `sys_wf_prof_ver_artifact_cf` table gone, the 5 vestigial `sys_wf_profile_ver_mst` columns gone, all 3 dependent FKs gone, both retired functions gone — `0` remaining on every check, both databases. Regenerated `web-admin/types/database.ts` (`npm run supabase:types`) and `database.generated.ts` (via Supabase MCP, then reconciled — both now 0 references to the dropped table and byte-identical). Re-ran the full affected surface post-apply: `workflow-engine.no-legacy`, `semantic-workflow-artifact`, `workflow-policy-resolver`, `workflow-engine.gates` (19 unit tests) plus `wf-prof-ver-validate-live`, `wf-live-policy-business-level-seed`, `delivery-completion`, `delivery-route-command` (46 DB-integration tests against the real migrated local DB) — all green. Soak remains before Gate 5 itself is fully closed; HQ Studio's `/compile` API retirement (`lwpr-hq-api`) is separate, HQ-side, still `in_progress`.

## Unreleased — 2026-09-05 (6)

- **Documented the legacy artifact-era `org_orders_mst` fields to prevent re-investigation.** Per (5) below, `wf_profile_artifact_id`/`wf_profile_revision`/`wf_profile_checksum`/`wf_profile_schema_version` are structurally wired into ~9 files (shared row-type shapes, `hasAnyBindingValue()`'s legacy-order safety net, and an audit passthrough into `org_wf_gate_decision_mst.profile_artifact_id`) without being functionally load-bearing for live policy resolution. Added a short "why" comment at every canonical type definition and use site so a future reader (including a future session) doesn't have to re-derive this from scratch: `semantic-workflow-artifact.service.ts` (`SemanticWorkflowOrderSnapshot`), `workflow-engine.service.ts` (`LockedOrderRow` + the `assertAndRecordSemanticGateDecisions` call site), `workflow-policy-resolver.service.ts` (`hasAnyBindingValue`), `workflow-gate-decision.service.ts` (`profileArtifactId` param), `delivery-completion.service.ts`, `pickup-completion.service.ts`, `workboard-query.service.ts`, `stage-worklist-query.service.ts`, `delivery-pod-method.service.ts`, `order-service.ts` (both create-order write sites). Comment-only, no behavior change. Gates: tsc clean, eslint clean, 12 suites / 59 tests green.
  **Tracked follow-up, not scheduled**: a full removal (rewrite ~9 files' SQL `SELECT`s/types, drop the `hasAnyBindingValue()` OR-term, drop the audit-write, then a migration dropping the 4 `org_orders_mst` columns) would fully resolve the ambiguity instead of just documenting it, but touches the gate-decision audit trail for pre-cutover legacy orders and is a separate, higher-touch piece of work — explicitly out of scope for Gate 5.

## Unreleased — 2026-09-05 (5)

- **Correction to (4) below**: that entry claimed the 4 `org_orders_mst` legacy-FK columns (`wf_profile_artifact_id`/`wf_profile_revision`/`wf_profile_checksum`/`wf_profile_schema_version`) are inert because "nothing reads that column, confirmed repo-wide." That check only covered reads of the `sys_wf_prof_ver_artifact_cf` **table** — it did not check these 4 **columns** on `org_orders_mst` directly, and they are in fact actively `SELECT`ed and typed in live code: `workflow-engine.service.ts`, `delivery-completion.service.ts`, `pickup-completion.service.ts`, `workboard-query.service.ts`, `stage-worklist-query.service.ts`, `semantic-workflow-artifact.service.ts`, `delivery-pod-method.service.ts`, and `workflow-policy-resolver.service.ts`'s `hasAnyBindingValue()` fail-closed check. `order-service.ts` still writes a real `wf_profile_revision` value on every order create. Migration `0494` was already correct in *behavior* (it only drops the FK, never touches these columns) — only the migration's own comment and the prior CHANGELOG entry mischaracterized why that was safe. Comment corrected in the migration file. **No columns should be dropped from `org_orders_mst`** as part of this retirement — that would require rewriting ~9 files' SQL/types for no functional gain and is out of scope.

## Unreleased — 2026-09-05 (4)

- **Fix: `0494_wf_prof_ver_artifact_retirement.sql` failed on first apply attempt.** Operator ran `supabase migration up` and hit `cannot drop table sys_wf_prof_ver_artifact_cf because other objects depend on it (2BP01)` — two FK constraints the original dependency check missed: `org_orders_mst.fk_ord_wf_prof_artifact` (a composite FK on `wf_profile_artifact_id`/`wf_profile_revision`/`wf_profile_schema_version`/`wf_profile_checksum`, separate from that table's live, still-used `fk_ord_wf_prof_ver_scope` binding — untouched) and `org_wf_gate_decision_mst.fk_wfgd_artifact`. Root cause: the original audit checked `pg_constraint` scoped to only the artifact table and `sys_wf_profile_ver_mst`, not a `confrelid`-based search across *every* table. Re-ran that full search on remote — confirmed exactly 3 FKs reference the table (the 2 missed ones plus the 1 already handled) and 0 views depend on it. Migration updated to drop both missed constraints before the table drop; still not applied.

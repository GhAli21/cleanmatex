...
| S10 | Staff POD delivery (routed) | On Delivery Stop Detail, complete an `out_for_delivery` stop with required POD evidence (not the generic Actions tab). | **SIGNED 2026-09-05.** Stop, POD, route counters, and `CONFIRM_DELIVERY` committed together; order became `delivered`; `delivered_at` populated. Real operator, `ORD-20260903-0005` on `WF_V2_SIMPLE` v4, POD method NOTES. Route/stop seeded via reviewed migration `0490` (`STAFF_DELIVERY_WRITES_ENABLED=false` still blocks normal route creation). Companion `delivery-completion.db.test.ts` fixed same session (was silently 4/9-failing since Gate 4, test-fixture gap not a service bug) — now 9/9 passing. |

---
version: v0.2.0
last_updated: 2026-02-07
author: CleanMateX AI Assistant
---

# folders_lookup

This file is a curated active-feature index used during the current documentation consolidation.

It does not yet represent every folder under `docs/features/`; until the full feature inventory is normalized, use this file together with `docs/README.md` and direct feature-folder navigation.

| Feature Name | Folder Path | Description | Version | Last Updated |
| --- | --- | --- | --- | --- |
| Order Service Preferences | `docs/features/Order_Service_Preferences/` | Service and packing preferences for orders (starch, perfume, hang, fold). Item and piece-level, customer standing prefs, Care packages, receipt placeholders. | v1.0.0 | 2026-03-12 |
| Cancel and Return Order | `docs/features/orders/cancel_return/` | Cancel order (before delivery) and Customer Return (after delivery) flows. Per-screen DB functions, refund voucher service, payment integration. | v1.0.0 | 2026-03-06 |
| Orders Workflow Migration | `docs/features/orders_workflow_migration/` | Migration of all order workflow screens to the new workflow system (screen contracts + transitions + gradual rollout flag). | v0.1.0 | 2026-01-15 |
| Inventory Stock Management | `docs/features/inventory_stock_management/` | Inventory stock tracking for retail/consumable items (detergents, hangers, bags, packaging). CRUD, stock adjustments, transaction history, KPI dashboard. | v1.0.0 | 2026-02-07 |
| B2B Feature | `docs/features/B2B_Feature/` | B2B corporate customers, contracts, credit limits, consolidated billing, statements, multi-contact, dunning. Gated by b2b_contracts. | v1.0.0 | 2026-03-13 |

---
name: cmx-api-backend-setup-and-rename
overview: Create a complete NestJS backend structure in cmx-api folder and update all references from "backend" to "cmx-api" across documentation, scripts, configs, and infrastructure files.
todos:
  - id: "1"
    content: "Create NestJS project structure in cmx-api: package.json, tsconfig.json, nest-cli.json, .eslintrc.js, .prettierrc, .env.example, .gitignore"
    status: pending
  - id: "2"
    content: "Create core application files: main.ts, app.module.ts, app.controller.ts, app.service.ts"
    status: pending
  - id: "3"
    content: "Create config module: config.module.ts, database.config.ts, redis.config.ts, app.config.ts"
    status: pending
  - id: "4"
    content: "Create common module: decorators (tenant-id), guards (jwt-auth, tenant), interceptors (logging, transform), pipes (validation), utils (logger)"
    status: pending
  - id: "5"
    content: "Create database module: database.module.ts, supabase.service.ts, tenant-context.service.ts"
    status: pending
  - id: "6"
    content: "Create health module: health.module.ts, health.controller.ts, health.service.ts"
    status: pending
  - id: "7"
    content: "Update package.json root workspaces: backend → cmx-api"
    status: pending
  - id: "8"
    content: "Update .clauderc root: directories.backend path"
    status: pending
  - id: "9"
    content: "Update cmx-api/.clauderc: backend references to cmx-api"
    status: pending
  - id: "10"
    content: "Update scripts: start-services.ps1, start-services.sh, start-services_x.ps1, cleanmatex_how_to_start_Jh.txt"
    status: pending
  - id: "11"
    content: "Update scripts: validate-env.js, feature-readme-template.md, consolidate-features.ps1"
    status: pending
  - id: "12"
    content: "Update root documentation: README.md, CLAUDE.md"
    status: pending
  - id: "13"
    content: "Update plan documentation: master_plan_cc_01.md, all plan_cr/*backend*.md files"
    status: pending
  - id: "14"
    content: "Update feature documentation: docs/features/*/README.md and related files"
    status: pending
  - id: "15"
    content: "Update development documentation: docs/dev/*.md files"
    status: pending
  - id: "16"
    content: "Update skills: .claude/skills/backend/*.md files"
    status: pending
  - id: "17"
    content: "Update rules: .cursor/rules/backend*.mdc files"
    status: pending
  - id: "18"
    content: Create cmx-api/README.md with setup instructions and architecture overview
    status: pending
  - id: "19"
    content: "Verify NestJS setup: build, start, health check, Swagger docs"
    status: pending
  - id: "20"
    content: Search and verify no remaining backend/ folder references (except historical/archived docs)
    status: pending
isProject: false
---
...
1. **Create NestJS structure** (Phase 1) - Foundation first
2. **Update package.json workspaces** (Phase 2.1) - Critical for monorepo
3. **Update scripts** (Phase 2.3) - Needed for development workflow
4. **Update root documentation** (Phase 2.2) - README.md, CLAUDE.md
5. **Update feature documentation** (Phase 2.2) - Systematic pass through docs/
6. **Update skills/rules** (Phase 2.4) - AI assistant context
7. **Verify and test** (Phase 3) - Ensure everything works
8. **Final documentation** (Phase 4) - Polish and completeness

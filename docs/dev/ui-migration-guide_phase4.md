...
```
@/components/ui/*  ──►  components/ui/*.tsx (re-exports)  ──►  @ui/compat/*
```

- **`components/ui/`** – Thin re-exports; every file delegates to `@ui/compat/`
- **`src/ui/compat/`** – Full implementations of legacy components (same API as before)
- **`@ui` barrel** – Exports `@ui/compat` plus all Cmx components (`CmxButton`, `CmxDialog`, etc.)

### Import Paths

| Import Path                       | Resolves To                                                        | Notes                     |
| --------------------------------- | ------------------------------------------------------------------ | ------------------------- |
| `@/components/ui`                 | `components/ui/index.ts` → `@ui/compat`                            | Legacy barrel             |
| `@/components/ui/Button`          | `components/ui/Button.tsx` → `@ui/compat/Button`                   | Per-file re-export        |
| `@/components/ui/select-dropdown` | `components/ui/select-dropdown.tsx` → `@ui/compat/select-dropdown` | Composable Select         |
| `@/components/ui/summary-message` | `components/ui/summary-message.tsx` → `@ui/compat/summary-message` | SummaryMessage            |
| `@ui`                             | `src/ui/index.ts`                                                  | Design system + compat    |
| `@ui/compat`                      | `src/ui/compat/index.ts`                                           | Legacy API only           |
